...
! This is the value used in CAM3 by Pritchard et al., GRL, 35, 2008.
  ...
  real(r8), public  :: albice(numrad) = &       ! albedo land ice by waveband (1=vis, 2=nir)
                       (/ 0.80_r8, 0.55_r8 /)
  logical, public :: rad_3d_topo = .false.  ! 3D topography effect on surface solar radiation by Lee
...
!!!!!!--- Added by Wei-Liang Lee ---------
  if (rad_3d_topo) then
    call Albedo_3D_Adjustment(lbp, ubp, num_nourbanp, filter_nourbanp, nextsw_cday)
  endif
!!!!!!--- End Lee ------------------------
...
!----------------------------------------------------------------------
  subroutine Albedo_3D_Adjustment(lbp, ubp, num_nourbanp, filter_nourbanp, &
                                  nextsw_cday)
...
!
    ...
    do fp = 1,num_nourbanp
       ...
       if (cosz > 0._r8 .and. abs(lat(g)) < 1.047_r8 .and. stdev_elev(g) > 0._r8) then
          local_timeofday = next_tod + lon(g) / pi * 180._r8 * 240._r8
          ...
          if (cosz == 1._r8) then
             ...
             azi_angle = (sin(lat(g))*cosz-sin(decl(c))) / (cos(lat(g))*sinz)
