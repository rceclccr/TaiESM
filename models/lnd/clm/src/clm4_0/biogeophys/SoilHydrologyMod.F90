...
!--------------------------NTUAS for irrigation 201507
  logical, public :: irri_climatology=.false.
  logical, public :: irri_transient=.false.
!--------------------------
...
! !USES:
    ...
    use clm_time_manager, only : get_step_size, get_curr_date, get_start_date !NTUAS for irrigation 
!
! !ARGUMENTS:
    implicit none
...
!
    ...
    integer :: cuyear,cumonth,cuday,cusecs     ! calendar info for current time step
    integer :: startyear,startmonth,startday,startsecs     ! calendar info for current time step
    real(r8), pointer :: irri4d(:,:,:) !(mm H2O /s)
    real(r8), pointer :: irri4d2(:,:) !(mm H2O /s)
    !--------------
...
!rtm_flood
    ...
    zi            => cps%zi
    irri4d        => cps%irri4d !NTUAS for irrigation
    irri4d2        => cps%irri4d2 !NTUAS for irrigation
    ! Get time step
    ...
    do fc = 1, num_hydrologyc
       ...
       !--------------------
       !NTUAS for irrigation, 201507, irri   
       call get_curr_date (cuyear, cumonth, cuday, cusecs)
       call get_start_date (startyear, startmonth, startday, startsecs)
       if (irri_climatology)then
           if (cuyear.gt.startyear) then
              qflx_top_soil(c)=qflx_top_soil(c) + irri4d2(c,cumonth)/30./86400.
           endif
       endif
       if (irri_transient)then
           if (cuyear.gt.1900) then
               if (cuyear.le.2000)then
                  qflx_top_soil(c)=qflx_top_soil(c) + irri4d(c,cuyear-1900,cumonth)/30./86400.
               endif
           endif
       endif
       !--------------------
...
! !USES:
    ...
    use clm_time_manager, only : get_curr_date,get_step_size,get_start_date!NTUAS for irrigation
!
! !ARGUMENTS:
    implicit none
    integer , intent(in) :: lbc, ubc                     ! column bounds
...
!NTUAS for irrigation
       ...
       if (irri_climatology)then
           if (cuyear.gt.startyear) then
                do fc = 1, num_hydrologyc
                   c = filter_hydrologyc(fc)
                   wa(c) = wa(c) -  irri4d2(c,cumonth)/30./86400.*dtime/4.
                   wt(c) = wt(c) -  irri4d2(c,cumonth)/30./86400.*dtime/4.
                end do
           endif
       endif
       if (irri_transient)then
           if (cuyear.gt.1900) then
               if (cuyear.le.2000)then
                   do fc = 1, num_hydrologyc
                      c = filter_hydrologyc(fc)
                      wa(c) = wa(c) -  irri4d(c,cuyear-1900,cumonth)/30./86400.*dtime/4.
                      wt(c) = wt(c) -  irri4d(c,cuyear-1900,cumonth)/30./86400.*dtime/4.
                   end do
               endif
           endif
       endif
