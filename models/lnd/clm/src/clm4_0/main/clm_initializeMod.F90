...
! !USES:
    ...
    use clm_varctl      , only : fsurdat, fatmlndfrc, flndtopo, fglcmask, noland, f3dtopo  ! Lee
    ...
    use surfrdMod       , only : surfrd_get_globmask, surfrd_get_grid, surfrd_get_topo, &
                                 surfrd_get_data, surfrd_get_3dtopo  ! Lee
    ...
    use SurfaceAlbedoMod, only : rad_3d_topo  ! Lee
!
! !ARGUMENTS:
!
! !REVISION HISTORY:
...
!-----------------------------------------------------------------------
    ...
    ! Get 3D topography parameters  ! Lee
