...
! !USES:
  ...
  use clm_varctl   , only : caseid, ctitle, nsrest, brnch_retain_casename, hostname, &
                            ...
                            fatmtopo, flndtopo, fpftdyn, fpftcon, nrevsn, f3dtopo, &  ! Lee
                            firriclima, firritransient, & ! NTUAS for irrigation
  ...
  use SurfaceAlbedoMod, only : albice, rad_3d_topo  ! Lee 3D
  use CNAllocationMod , only : suplnitro
!-------------NTUAS for irrigation
  use SoilHydrologyMod, only : irri_climatology, irri_transient
!-------------
...
!------------------------------------------------------------------------
    ...
    ! 3D topography on radiation options  ! Lee
    namelist /clm_inparm/  &
         rad_3d_topo, f3dtopo

    ! NTUAS for irrigation 201507
    namelist /clm_inparm/  &
        irri_climatology, firriclima, irri_transient, firritransient
...
!-----------------------------------------------------------------------
    ...
    call mpi_bcast (f3dtopo , len(f3dtopo) , MPI_CHARACTER, 0, mpicom, ier)  ! Lee
    !-- NTUAS for irrigation
    call mpi_bcast (firriclima,  len(firriclima), MPI_CHARACTER, 0, mpicom, ier)
    call mpi_bcast (firritransient,  len(firritransient), MPI_CHARACTER, 0, mpicom, ier)
    !---------------
    ...
    call mpi_bcast (rad_3d_topo , 1, MPI_LOGICAL, 0, mpicom, ier)
    call mpi_bcast (irri_climatology , 1, MPI_LOGICAL, 0, mpicom, ier) !NTUAS for irrigation
    call mpi_bcast (irri_transient , 1, MPI_LOGICAL, 0, mpicom, ier) !NTUAS for irrigation
...
!------------------------------------------------------------------------
    ...
    !--------------------
    if (irri_climatology) then  ! NTUAS for irrigation
      if (firriclima == ' ') then
         call endrun(subname//' ERROR: firriclima not set while irri_climatology is True')
      else
         write(iulog,*) '   firriclima data = ',trim(firriclima)
      end if
    else
       write(iulog,*) '   irri_climatology is False, so do not run irri_climatology'
    end if

    if (irri_transient) then  ! NTUAS for irrigation
      if (firritransient == ' ') then
         call endrun(subname//' ERROR: firritransient not set while irri_transient is True')
      else
         write(iulog,*) '   firritransient data = ',trim(firritransient)
      end if
    else
       write(iulog,*) '   irri_transient is False, so do not run irri_transient'
    end if
    !--------------------
    if (use_cn) then
    ...
    write(iulog,*) '   3D topography on solar radiation   = ', rad_3d_topo
    !---------------NTUAS for irrigation
    write(iulog,*) '   climatological irrigation =', irri_climatology
    write(iulog,*) '   transient irrigation =', irri_transient
    !------------------
