...
!--- NTUAS for irrigation 201507
  real(r8), pointer :: irri4d(:,:,:), irri4d2(:,:), irri_tmp(:,:)
  integer :: iindex, iyr, imn
!---------------------
...
!------------------------------------------------------------------------
  ...
  if (irri_transient) then
     allocate(irri_tmp(begg:endg,1200))
  endif
  if (irri_climatology) then
     allocate(irri_tmp(begg:endg,12))
  endif
  !--------
...
!-------------------
! Read IRRI NTUAS for irrigation, 201507
  if (irri_climatology) then
     call getfil (firriclima,locfn,0) 
     call ncd_pio_openfile (ncid,locfn,0)
     call ncd_io(ncid=ncid, varname='IRRI4d', flag='read', data=irri_tmp, dim1name=grlnd, readvar=readvar)
     call ncd_pio_closefile(ncid)
  endif
  if (irri_transient) then
     call getfil (firritransient,locfn,0) 
     call ncd_pio_openfile (ncid,locfn,0)
     call ncd_io(ncid=ncid, varname='IRRI4d', flag='read', data=irri_tmp, dim1name=grlnd, readvar=readvar)
     call ncd_pio_closefile(ncid)
  endif
...
!--------------------
  ...
  ! --------------------------------------------------------------------
   ...
   do c = begc, endc
      ...
      isoicol(c) = soic2d(g)
!-------------------------
!NTUAS for irrigation, change to year, mn dim, 201507
  if (irri_transient) then
    iindex = 0
    do iyr = 1, 100
    do imn = 1, 12
      iindex = iindex + 1
      irri4d(c,iyr,imn) = irri_tmp(g,iindex)
    enddo
    enddo
  endif
  if (irri_climatology) then
    ...
    do imn = 1, 12
      iindex = iindex + 1
      irri4d2(c,imn) = irri_tmp(g,iindex)
    enddo
  endif
!-------------------------
...
! endif
   ...
   if (irri_transient) deallocate(irri_tmp) !NTUAS for irrigation
   if (irri_climatology) deallocate(irri_tmp) !NTUAS for irrigation
