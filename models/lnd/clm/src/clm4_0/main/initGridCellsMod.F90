...
!           ltype=isturb, wtxy=wtxy, vegxy=vegxy,   &
      ...
      elseif (ldomain%latc(gdc) < -60.) then
       ...
       if (my_gcell) then
          ...
          gptr%stdev_elev(gdc)     = ldomain%stdev_elev(gdc)
          gptr%sky_view(gdc)       = ldomain%sky_view(gdc)
          gptr%terrain_config(gdc) = ldomain%terrain_config(gdc)
          gptr%sinsl_cosas(gdc)    = ldomain%sinsl_cosas(gdc)
          gptr%sinsl_sinas(gdc)    = ldomain%sinsl_sinas(gdc)
       endif
