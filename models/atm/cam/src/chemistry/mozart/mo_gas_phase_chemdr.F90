module mo_gas_phase_chemdr
...
#ifdef MODAL_AERO
  integer :: ndx_dgnum, ndx_dgnumwet, ndx_wetdens_ap, sgnum_idx, lnsgnum_idx ! add sgn by ICTsai
  ...
  character(len=fieldname_len), dimension(ntot_amode) :: sgnum_name
#endif
...
! add sfc by ICTsai
      sgnum_name(n) = ' '
      write(sgnum_name(n),fmt='(a,i1)') 'sigmag',n
      call addfld( sgnum_name(n), 'm', pver, 'I', 'Aerosol mode sigma', phys_decomp )
    end do
#endif
...
! add sgn by ICTsai
    sgnum_idx      = pbuf_get_index('SGNUM')
!    lnsgnum_idx    = pbuf_get_index( 'LNSGNUM' )
!
#endif
...
!
    ...
    real(r8), pointer :: sgnum(:,:,:)
!    real(r8) :: lnsgnum(ncol,pver,ntot_amode)
!    real(r8):: sgnum(ncol,pver,ntot_amode),lnsgnum(ncol,pver,ntot_amode)
...
! add sgn by ICTsai
    call pbuf_get_field(pbuf, sgnum_idx,      sgnum,  start=(/1,1,1/), kount=(/pcols,pver,ntot_amode/) )
!    call pbuf_get_field(pbuf, lnsgnum_idx,      lnsgnum,  start=(/1,1,1/), kount=(/pcols,pver,ntot_amode/) )
!
    do n=1,ntot_amode
       call outfld(dgnum_name(n),dgnumwet(1:ncol,1:pver,n), ncol, lchnk )
       call outfld(sgnum_name(n),sgnum(1:ncol,1:pver,n), ncol, lchnk )
    end do
