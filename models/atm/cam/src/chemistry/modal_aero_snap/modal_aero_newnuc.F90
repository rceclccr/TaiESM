...
!   local variables
	...
	integer :: lnumait, lso4ait, lnh4ait,lsfcait
...
!       integer, parameter :: newnuc_method_flagaa = 12
	...
	real(r8) :: dadt_ait, dadt_aitsv1, dadt_aitsv2, dadt_aitsv3 !ICTsai
...
! variable for dadt by ICTsai
        real(r8), parameter :: d20 = 2.0E-07  ! diameter of a new particle [cm]
        real(r8), parameter :: d20sq = d20*d20  ! new-particle diameter squared [cm**2]
        real(r8), parameter :: m2_20 = 1.0e-4 * d20sq ! new-particle diameter squared [m**2]
...
!   production rate of 2nd moment [ m**2 /kmol-air ] by ICTsai
!        dadt_ait = max(0.0_r8, m2_20*dndt_ait)
        dadt_ait = max(0.0_r8, m2_20*dndt_ait)
...
!temporary test by ICTsai
!        dndt_ait=0.0_r8
!        dadt_ait=0.0_r8
!        dmdt_ait=0.0_r8

	dndt_aitsv1 = dndt_ait
	dadt_aitsv1 = dadt_ait   !by ICTsai
...
! add sfc by ICTsai
        if (lsfcait > 0) then
            dqdt(i,k,lsfcait) = dadt_ait*(1.0_r8-cldx)
            qsrflx(i,lsfcait,1) = qsrflx(i,lsfcait,1)   &
                                + dqdt(i,k,lsfcait)*pdel_fac
            q(i,k,lsfcait) = q(i,k,lsfcait) + dqdt(i,k,lsfcait)*deltat
!            write(*,*)'NEW ICT',q(i,k,lsfcait),dqdt(i,k,lsfcait),q(i,k,lnumait)&
!               ,dqdt(i,k,lnumait)
        end if
!
...
!   diagnostic output start ----------------------------------------
 	...
 	if ((k >= 24) .or. (mod(k,4) == 0)) then
        ...
        write(lun,97020) 'dndt_,dmdt_aitsv1,dadt_aitsv1',   &
 		 dndt_aitsv1, dmdt_aitsv1,dadt_aitsv1
        write(lun,97020) 'dndt_,dmdt_aitsv2,dadt_aitsv2',   &
 		 dndt_aitsv2, dmdt_aitsv2,dadt_aitsv2
        write(lun,97020) 'dndt_,dmdt_aitsv3,dadt_aitsv3 ',   &
 		 dndt_aitsv3, dmdt_aitsv3,dadt_aitsv3
        write(lun,97020) 'dndt_, dmdt_ait,dadt_ait      ',   &
 			 dndt_ait, dmdt_ait,dadt_ait
...
! calc h2so4 in molecules/cm3 and nh3 in ppt
        ...
        so4vol_in  = qh2so4_avg * cair * avogad * 1.0e-6_r8  !molecules/cm3
...
!              write(*,'(A9,20E13.5)')'CESM NUCP',ratenuclt
!  add SNAP by ICTsai
               call binary_nuc_snap(   &
                  temp_bb, rh_bb, so4vol_bb,   &
                  ratenuclt, rateloge,   &
                  cnum_h2so4, cnum_tot, radius_cluster )
!              write(*,'(A9,20E13.5)')'SNAP NUCP',ratenuclt
...
!        ratenuclt_kk = ratenuclt_bb*factor_kk
        ratenuclt_kk = ratenuclt_bb*factor_kk*0.5_r8 !*0.5: adjust due to large time step by ICTsai
...
!        write(*,'(A9,10E13.5)') 'CESM CRIT',crit_x
...
!-----------------------------------------------------------------------
        subroutine binary_nuc_snap( temp, rh, so4vol,   &
            ratenucl, rateloge,   &
            cnum_h2so4, cnum_tot, radius_cluster )
!
! calculates binary nucleation rate and critical cluster size
! using the parameterization in
!    modified following Chen et al. (2013) ACP by ICTsai
...
! local variables
        real(r8) :: crit_x,crit_x1
        real(r8) :: acoe, bcoe, ccoe, dcoe, ecoe, fcoe, gcoe, hcoe, icoe, jcoe
        real(r8) :: tmpa, tmpb
        real(r8) :: acoe1, bcoe1, ccoe1, dcoe1, ecoe1, fcoe1, gcoe1, hcoe1, icoe1, jcoe1
        real(r8) :: fpssa1,fpssa2,fpssa,fpsa,ssa,kufpssa1,kufpssa,ssa2
...
! calc sulfuric acid mole fraction in critical cluster
        crit_x1 = 0.740997_r8 - 0.00266379_r8 * temp   &
               - 0.00349998_r8 * log (so4vol)   &
               + 0.0000504022_r8 * temp * log (so4vol)   &
              + 0.00201048_r8 * log (rh)   &
              - 0.000183289_r8 * temp * log (rh)   &
              ...
              - 1.50345e-6_r8 * temp * (log (rh)) ** 3.0_r8
! calc sulfuric acid mole fraction in critical cluster 
         fpsa=so4vol*1.e6_r8*8.314_r8*temp/6.023e+23_r8  !p_sa

! .....  SATURATION VAPOR PRESSURE OF SULFURUC ACID  .....
! .....  from: Seinfeld and Pendis, 1998, p. 577

         fpssa1=360.15_r8/temp
         fpssa2=-1._r8/temp+2.778e-3_r8+6.9725E-4_r8*(1._r8+LOG(fpssa1)-fpssa1)
         fpssa=EXP( -11.952_r8+10156._r8*fpssa2 )
         fpssa=fpssa*101325._r8   !   

         kufpssa1=1._r8+log(360.15_r8/temp)-(360.15_r8/temp)
         kufpssa=101325._r8*exp(-11.695_r8+10156._r8*(1._r8/360.15_r8-1._r8/temp&
           +0.38_r8*kufpssa1/545._r8)) !pa
     
         acoe1=-6.8190353_r8+0.025746829_r8*temp+1070.618_r8/temp  &
           -4.184687e-5_r8*temp*temp &
           -61228.947_r8/temp/temp+3.004042e-8_r8*temp*temp*temp
!         bcoe1=-0.02095-1.59219E-9*temp*temp*temp
         bcoe1=-0.02095_r8-1.59219e-10_r8*temp*temp*temp
         ccoe1=0.0041_r8+2.68278e-10_r8*temp*temp*temp
         dcoe1=0.00174_r8-3.3858e-10_r8*temp*temp*temp
         ecoe1=8.43580e-5_r8-1.3835e-11_r8*temp*temp*temp
         fcoe1=exp(-8.11399_r8+2.46303e-5_r8*temp*temp)

         ssa=max(fpsa/fpssa,1.e-30_r8)
         ssa2=max(fpsa/kufpssa,1.e-30_r8)
!         write(*,*)'FPSSA',ssa,ssa2,fpssa,kufpssa

         crit_x =acoe1+bcoe1*log(rh)+ccoe1*log(ssa)+dcoe1*log(rh)  &
          *log(rh)+ecoe1*log(ssa)*log(ssa)+fcoe1*log(rh)*log(ssa)
         crit_x=max(1.e-10_r8,crit_x*0.4_r8)
!         write(*,'(A9,10E13.5)') 'SNAP CRIT',ssa,rh,crit_x

!         write(6,*)'binary',temp,rh,ssa,0.02574*temp,1070.618/temp, &
!             -4.184e-5*temp*temp, -61228.947/temp/temp,3.004e-8*temp*temp*temp&
!            ,acoe1,crit_x
...
!   calc radius (nm) of critical cluster
      ...
      return
      end subroutine binary_nuc_snap
...
!   skip if no aitken mode so4 or num species
	...
	if (mait > 0) then
	    ...
	    lsfcait = lsfcptr_amode(mait)  !ICTsai
	...
	if ((l_h2so4  <= 0) .or. (l_h2so4 > pcnst)) then
	    ...
	    return
!add sfc by ICTsai
        else if ((lsfcait <= 0) .or. (lsfcait > pcnst)) then
            write(*,'(/a/)')   &
                '*** modal_aero_newnuc bypass -- lsfcait <= 0'
            return
!
