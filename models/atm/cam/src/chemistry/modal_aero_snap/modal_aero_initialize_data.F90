...
contains
  ...
  subroutine modal_aero_register
    ...
    character(len=*), parameter ::     xname_numptrcw(ntot_amode) = (/ 'num_c1  ', 'num_c2  ', &
         'num_c3  ' /)
! add prognostic sfc by ICTsai
    character(len=*), parameter ::     xname_lsfcptr(ntot_amode)   = (/ 'sfc_a1  ', 'sfc_a2  ', &
         'sfc_a3  ' /)
    character(len=*), parameter ::     xname_lsfcptrcw(ntot_amode) = (/ 'sfc_c1  ', 'sfc_c2  ', &
         'sfc_c3  ' /)
#endif
...
! define sfc variables by ICTsai
          call search_list_of_names(                                  &
               xname_lsfcptr(m), lsfcptr_amode(m), cnst_name, pcnst )
          if (lsfcptr_amode(m) .le. 0) then
             write(iulog,9062) 'xname_sfcptr', xname_lsfcptr(m), m
             call endrun()
          end if
          species_class(lsfcptr_amode(m)) = spec_class_aerosol

          lsfcptrcw_amode(m) = lsfcptr_amode(m)  !use the same index for Q and QQCW arrays
          if (lsfcptrcw_amode(m) .le. 0) then
             write(iulog,9062) 'xname_sfcptrcw', xname_lsfcptrcw(m),  m
             call endrun()
          end if
         call pbuf_add_field(xname_lsfcptrcw(m), 'global', dtype_r8, (/pcols,pver/), iptr)
         call qqcw_set_ptr(lsfcptrcw_amode(m),iptr)

          species_class(lsfcptrcw_amode(m)) = spec_class_aerosol
!
       ...
       if ( masterproc ) then
          ...
          write(iulog,9233) 'sfcptr         ',                           &
               lsfcptr_amode(m), xname_lsfcptr(m)
          write(iulog,9233) 'sfcptrcw       ',                           &
               lsfcptrcw_amode(m), xname_lsfcptrcw(m)
...
!       real(r8) :: hygro_aer
       ...
       do m = 1, ntot_amode
          call rad_cnst_get_mode_props(0, m, &
             ...
             dgnumhi=dgnumhi_amode(m), rhcrystal=rhcrystal_amode(m), rhdeliques=rhdeliques_amode(m),sgnum=sgnum_amode(m),sigmaglo=sigmaglo_amode(m),sigmaghi=sigmaghi_amode(m)) !add sgnum by ICTsai
...
!          alnsg_amode(m) = log( sigmag_amode(m) )
          alnsg_amode(m) = log( sgnum_amode(m) ) ! prognostic sigma by ICTsai
          !
!          print*,'dgnumlo ICTsai', m, sgnum_amode(m),sigmaglo_amode(m),sigmaghi_amode(m)
       ...
       ! deinfe sfc by ICTsai
       voltosfc_amode(m) = 1._r8 / ( (pi/2._r8)*                            &
            (dgnum_amode(m)**2._r8)*exp(2._r8*alnsg_amode(m)**2._r8) )
       voltosfclo_amode(m) = 1._r8 / ( (pi/2._r8)*                          &
            (dgnumlo_amode(m)**2._r8)*exp(2._r8*alnsg_amode(m)**2._r8) )
       voltosfchi_amode(m) = 1._r8 / ( (pi/2._r8)*                          &
            (dgnumhi_amode(m)**2._r8)*exp(2._r8*alnsg_amode(m)**2._r8) )
       end do
...
!          end do
!          if(ibulk.eq.0)then
!             write(iulog,*) 'modal species names do not match bulk names for modal species ',specname_amode(l),bulkname,l,iaerosol,numaerosols
...
!          call addfld( &
!               'dgnd_a'//trnum(3:3), 'm', pver, 'A', &
!               'dry dgnum, interstitial, mode '//trnum(3:3), phys_decomp )
!          call addfld( &
!               'dgnw_a'//trnum(3:3), 'm', pver, 'A', &
!               'wet dgnum, interstitial, mode '//trnum(3:3), phys_decomp )
!          call addfld( &
!               'wat_a'//trnum(3:3), 'm', pver, 'A', &
!               'aerosol water, interstitial, mode '//trnum(3:3), phys_decomp )
          !add sgn by ICTsai
          call addfld( &
               'sgnd_a'//trnum(3:3), 'm', pver, 'A', &
               'sigma, interstitial, mode '//trnum(3:3), phys_decomp )

!          if ( history_aerosol ) then    
!            call add_default( 'dgnd_a'//trnum(3:3), 1, ' ' )
!            call add_default( 'dgnw_a'//trnum(3:3), 1, ' ' )
!            call add_default( 'wat_a'//trnum(3:3),  1, ' ' )     
            call add_default( 'sgnd_a'//trnum(3:3), 1, ' ' )
!          endif
...
9241   format( i4, 2( 2x, i12, 2x, a ),                                &
     ...
     subroutine initaermodes_set_cnstnamecw
       ...
       cnst_name_cw = ' '
       do m = 1, ntot_amode
          do ll = -1, nspec_amode(m)
             if (ll == -1) then
                la = numptr_amode(m)
                lc = numptrcw_amode(m)
             else if (ll == 0) then
                la = lsfcptr_amode(m)
                lc = lsfcptrcw_amode(m)
             else
