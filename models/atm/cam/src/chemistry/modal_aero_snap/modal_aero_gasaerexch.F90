...
!add sgn by ICTsai
   real(r8), intent(in)    :: sgncur_a(pcols,pver,ntot_amode)
!   real(r8), intent(in)    :: lnsgncur_a(pcols,pver,ntot_amode)
...
!     method_soa=2 is reversible uptake using subr modal_aero_soaexch
   ...
   integer :: ido_sfc(ntot_amode)
   integer :: jac, jsrf
   integer :: k
   integer :: l, l2, lb, lsfrm, lstoo
   integer :: l_so4g, l_nh4g, l_msag, l_soag,l_sfca1,l_sfca2
   ...
   real (r8) :: fgain_sfc(ntot_amode) !ICTsai
   real (r8) :: g0_soa
   real (r8) :: pdel_fac
   ...
   logical  :: do_sfca1         ! true if soa gas is a species
   logical  :: do_sfca2         ! true if soa gas is a species
...
! set gas species indices
   ...
   call cnst_get_ind( 'sfc_a1',  l_sfca1, .false. )  !add sfc by ICTsai
   call cnst_get_ind( 'sfc_a2',  l_sfca2, .false. )  !ICTsai
   l_so4g = l_so4g - loffset
   l_nh4g = l_nh4g - loffset
   l_msag = l_msag - loffset
   l_soag = l_soag - loffset
! get sfc variables, ICTsai
   l_sfca1 = l_sfca1 - loffset
   l_sfca2 = l_sfca2 - loffset
!   write(*,*)'sfc get ICT',l_sfca1,l_sfca2
...
! add sfc by ICTsai
   if ((l_sfca1 > 0) .and. (l_sfca1 <= pcnstxx)) do_sfca1 = .true.
   if ((l_sfca2 > 0) .and. (l_sfca2 <= pcnstxx)) do_sfca2 = .true.
...
! sfc by ICTsai
             fgain_sfc(n)= uptkrate2(n,i,k)
           if (ido_sfc(n) > 0 ) then
             qold_sfc(n)=q(i,k,lsfcptr_amode(n)-loffset)
             sum_uprt_sfc=sum_uprt_sfc+fgain_sfc(n)
...
!        avg_uprt_sfc = (1.0 - exp(-deltatxx*sum_uprt_sfc))/deltatxx  !ICTsai
...
!        sum_dqdt_sfc = q(i,k,l_sfc) * avg_uprt_sfc !ICTsai
        if ( do_msag ) then
...
!            dqdt_sfc(n) = fgain_sfc(n)*(sum_dqdt_sfc)   !ICTsai
...
!! do temporary sfc changes by ICTsai
            if (ido_sfc(n) == 1) then
            l=lsfcptr_amode(n)-loffset 
            if (ido_so4a(n) == 1) then
            dqdt(i,k,l) = dqdt_so4(n)
...
!           write(*,*)'UPTAKE ICT',l,q(i,k,l),dqdt(i,k,l),deltat
         end do
         end do
      end if
...
!   do history file column-tendency fields
	...
	do jac = 1, 2
!            write(*,*)'NAME ICT',lb,cnst_name(lb), trim(cnst_name(lb))
...
subroutine gas_aer_uptkrates( ncol,       loffset,                &
                              q,          t,          pmid,       &
                              dgncur_awet,            uptkrate,   &  !ICTsai
                              uptkrate2, sgncur_awet   )
...
implicit none
   ...
   real(r8), intent(in) :: dgncur_awet(pcols,pver,ntot_amode)
   real(r8), intent(in) :: sgncur_awet(pcols,pver,ntot_amode)
!   real(r8), intent(in) :: lnsgncur_awet(pcols,pver,ntot_amode)

   real(r8), intent(out) :: uptkrate(ntot_amode,pcols,pver)  
                            ! gas-to-aerosol mass transfer rates (1/s)
   real(r8), intent(out) :: uptkrate2(ntot_amode,pcols,pver)  !m2,ICTsai
...
! local variables for PAP by ICTsai
   real(r8) :: rgmean,freepath
   real(r8) :: para0,para1,para2,para3,para_k3
   real(r8) :: betapap,b1,b2,tmp2,tmp3
   real(r8) :: fpara
...
!        write(*,'(A10,10E13.5)')'  UPTAKE ',dgncur_awet(i,k,n),sigmag_amode(n),num_a &
!          ,q(i,k,numptr_amode(n)-loffset),uptkrate(n,i,k)
! PAP by ICTsai,2012/05/20
        tmp2=gasdiffus
        tmp3=4.*gasdiffus/0.65/gasspeed
        rgmean=dgncur_awet(i,k,n)/2.
        knudsen = freepathx2/dgncur_awet(i,k,n)
!        write(*,*)'ICT gas1',tmp2,tmp3,rgmean,dgncur_awet(i,k,n),knudsen

!g1,2 and g1,3 in Chen et al. (2013)
!        para2=exp(log(sigmag_amode(n))*(-0.70413-0.12431*exp(-log(knudsen))+&
!          0.96979*log(sigmag_amode(n))))
!        para3=exp(log(sigmag_amode(n))*(0.66771-0.0415*exp(-log(knudsen))+&
!          0.97147*log(sigmag_amode(n))))
! release sigmag
        para2=exp(log(sgncur_awet(i,k,n))*(-0.70413-0.12431*exp(-log(knudsen))+&
          0.96979*log(sgncur_awet(i,k,n))))
        para3=exp(log(sgncur_awet(i,k,n))*(0.66771-0.0415*exp(-log(knudsen))+&
          0.97147*log(sgncur_awet(i,k,n))))
!         write(*,*)'ICT gas2',para2,para3

!Fuchs and Sutugin(1970)
        fuchs_sutugin = (0.75*0.65*(1. + knudsen*3./2.)) /   &
                  (knudsen*3./2.*(1.184 + knudsen*3./2.) + 0.4875)

        b1=tmp2*fuchs_sutugin/rgmean      
        aircon = rhoair/mwdry              ! (kmol-air/m3)
        num_a = q(i,k,numptr_amode(n)-loffset)*aircon
!        write(*,*)'ICT gas3',b1,fuchs_sutugin,b1,num_a

        uptkrate2(n,i,k)=2.*b1*num_a*2.*rgmean*para2  !M2
        uptkrate(n,i,k)=2.*b1*num_a*3.*rgmean**2*para3  !M3
...
!function for PAP condensation by ICTsai
   REAL FUNCTION fpara(a,b,c,d)
   real(r8),intent(in):: a,b,c,d
   real(r8):: f1,f2,f3,alpha,beta1,gama
   real(r8), parameter :: cpi = 3.14159
      ...
      f3=exp(d*d-f1*f1/2./f2)
      fpara=sqrt(cpi)*f3/2./alpha
   END FUNCTION fpara
!end of function for PAP
...
!     integer,  intent(in)  :: idiagss
      ...
      real(r8), parameter :: alpha = 0.05  ! parameter used in calc of time step
      real(r8), parameter :: g_min1 = 1.0e-20
      real(r8), parameter :: opoa_frac = 0.1  ! fraction of poa that is opoa
      ...
      real(r8), parameter :: rgas = 8.3144   ! gas constant in J/K/mol
...
! local
   ...
   integer  :: l_sfca1,l_sfca2,l_sfca3   !ICTsai
   integer  :: m, mfrm, mtoo
   integer  :: nsamefrm, nsametoo, nspec
   integer  :: n, nacc, nait

   logical  :: do_msag, do_nh4g, do_soag
   logical  :: do_sfca1, do_sfca2 ,do_sfca3  !ICTsai
...
! set gas species indices
      ...
      call cnst_get_ind( 'MSA',   l_msag, .false. )
      call cnst_get_ind( 'SOAG',  l_soag, .false. )
      call cnst_get_ind( 'sfc_a1',  l_sfca1, .false. )   !ICTsai
      call cnst_get_ind( 'sfc_a2',  l_sfca2, .false. )   !ICTsai
      call cnst_get_ind( 'sfc_a3',  l_sfca3, .false. )   !ICTsai
...
! set tendency flags
      dotend(:) = .false.
      dotend(l_so4g) = .true.
      dotend(l_sfca1) = .true.    !ICTsai
      dotend(l_sfca2) = .true.    !ICTsai
      dotend(l_sfca3) = .true.    !ICTsai
      if ( do_nh4g ) dotend(l_nh4g) = .true.
      if ( do_msag ) dotend(l_msag) = .true.
      if ( do_soag ) dotend(l_soag) = .true.
      do n = 1, ntot_amode
...
!        write(*,*)'PCNST ICT', l,dotend(l),fieldname,unit
