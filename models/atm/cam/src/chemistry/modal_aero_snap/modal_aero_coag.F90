...
! local variables
	...
	integer :: iselfcoag2done(ntot_amode)
	integer :: jfreqcoag
	integer :: k
...
!	logical, parameter :: fastcoag_flag = .false. ! selects coag rate-coef method
	...
	real(r8) :: xsfcconc(ntot_amode), xsfcconcnew(ntot_amode) !ICTsai
	real(r8) :: ybetaij0(maxpair_acoag), ybetaij3(maxpair_acoag)
	real(r8) :: ybetaii0(maxpair_acoag), ybetajj0(maxpair_acoag)
	real(r8) :: ybetaij2i(maxpair_acoag), ybetaij2j(maxpair_acoag),&   !ICTsai
                    ybetaii2(maxpair_acoag), ybetajj2(maxpair_acoag)       !ICTsai
...
!  if (ncol /= -999888777) return
   if (nstep > 10) call endrun( 'modal_aero_coag_sub -- nstep>10 testing halt' )
   end if   ! (ldiag1 > 0)
!--------------------------------------------------------------------------------
...
!   calculate 2nd moment conc. (m2/m3) for modes doing coagulation,ICTsai
        do n = 1, ntot_amode
            if (idomode(n) .gt. 0) then
                xsfcconc(n) = q(i,k,lsfcptr_amode(n)-loffset)*aircon
                xsfcconc(n) = max( 1.e-30_r8, xsfcconc(n) )
            end if
            iselfcoag2done(n) = 0
        end do
...
!
        call getcoags_wrapper_f(                                       &
          ...
          xbetaii0, xbetaii2,  xbetajj0,  xbetajj2                     )
!         write(*,'(A9,20E13.5)')'CMAQ DGAR',dgncur_awet(i,k,modefrm),dgncur_awet(i,k,modetoo),&
!        sigmag_amode(modefrm),sigmag_amode(modetoo),wetdens_a(i,k,modefrm),wetdens_a(i,k,modetoo)

!         write(*,'(A9,2I4,20E13.5)')'SNAP COAG', i,k,xbetaij0, xbetaij2i, xbetaij2j, xbetaij3,&
!            xbetaii0, xbetaii2,  xbetajj0,  xbetajj2
!        else
!
! compute coagu rate by PAP ICTsai(2012)
!
        wetdgnum_frm = dgncur_awet(i,k,modefrm)/dgncur_a(i,k,modefrm)
        wetdgnum_too = dgncur_awet(i,k,modetoo)/dgncur_a(i,k,modetoo)

        curdgn_frm   = dgncur_a(i,k,modefrm)
        curdgn_too   = dgncur_a(i,k,modetoo)
         
        wetdens_frm  = wetdens_a(i,k,modefrm) !convert unit to g/cm3
        wetdens_too  = wetdens_a(i,k,modetoo) !convert unit to g/cm3
        ...
        lnsg_too = alnsg_amode(modetoo)
         ...
         call calc_coag_snap(                                        &
          t(i,k), pmid(i,k),iok,                 &
           ...
           curdgn_frm, curdgn_too,               &
           lnsg_frm,lnsg_too,wetdens_frm,wetdens_too,  &
           xbetaij0, xbetaij2i, xbetaij2j, xbetaij3,                    &
          xbetaii0, xbetaii2,  xbetajj0,  xbetajj2                     ) 
!         write(*,'(A9,20E13.5)')'SNAP CGAR',  wetdgnum_frm,wetdgnum_too,&
!          sg_frm,sg_too  ,wetdens_frm,wetdens_too
!         write(*,'(A9,20E13.5)')'SNAP COAG', xbetaij0, xbetaij2i, xbetaij2j, xbetaij3,&
!            xbetaii0, xbetaii2,  xbetajj0,  xbetajj2
...
!	ybetaij0(ipair) = 0._r8
!	ybetaij3(ipair) = 0._r8
!	ybetaii0(ipair) = 0._r8
!	ybetajj0(ipair) = 0._r8
! 2nd moment by ICTsai
        ybetaij2i(ipair)=xbetaij2i
        ybetaij2j(ipair)=xbetaij2j 
        ybetaii2(ipair)=xbetaii2 
        ybetajj2(ipair)=xbetajj2
!        ybetaij2i(ipair)=xbetaij0**(2.)
!        ybetaij2j(ipair)=xbetaij0**(2.)
!        ybetaii2(ipair)=xbetaii0**(2.)
!        ybetajj2(ipair)=xbetajj0**(2.)
!end 2nd moment
...
! calculate 2nd moment changes in 'too' mode by ICTsai
       if ( (mprogsfc_amode(modetoo) > 0) .and.   &
             (iselfcoag2done(modetoo) <= 0) ) then
            iselfcoag2done(modetoo) = 1
            tmpn = xnumbconc(modetoo)
            tmps = xsfcconc(modetoo)
            dumprod= ybetaij2j(ipair)*tmpn*tmpn  
            dumloss= (ybetajj2(ipair)*tmpn*tmpn)/tmps
           if (dumloss.gt.0.0_r8) then
            dumpol=dumprod/dumloss
            xsfcconcnew(modetoo) = dumpol+(tmps-dumpol)*exp(-dumloss*deltat)
           else
            xsfcconcnew(modetoo)=tmps+dumprod*deltat
           endif
            lstoo = lsfcptr_amode(modetoo) - loffset
            q(i,k,lstoo) = max(1.e-30_r8,xsfcconcnew(modetoo)/aircon)
           
            dqdt(i,k,lstoo) = (xsfcconcnew(modetoo)-tmps)*deltatinv_main/aircon
!            write(*,*)'COAG M21ICT_1',q(i,k,lstoo),tmpn,tmps,dumprod,dumloss
!            write(*,*)'RATE ICT',ybetaij2j(ipair),ybetajj2(ipair),&
!               ybetaij2i(ipair),ybetaii2(ipair)

       endif
        if ( (mprogsfc_amode(modefrm) > 0) .and.   &
             (iselfcoag2done(modefrm) <= 0) ) then
            iselfcoag2done(modefrm) = 1
            tmpn = xnumbconc(modefrm)
            tmps = xsfcconc(modefrm)
            tmpa = ybetaij2i(ipair)*xnumbconc(modetoo)
            tmpb = ybetaii2(ipair)*tmpn
            tmpc = (tmpa + tmpb)*tmpn/tmps
            if (tmpc .gt. 0._r8) then
                xsfcconcnew(modefrm) = tmps*exp(-tmpc*deltat)
            else
                xsfcconcnew(modefrm) = tmps
            end if
            lsfrm = lsfcptr_amode(modefrm) - loffset
            q(i,k,lsfrm) = max(1.e-30_r8,xsfcconcnew(modefrm)/aircon)
            dqdt(i,k,lsfrm) = (xsfcconcnew(modefrm)-tmps)*deltatinv_main/aircon
!            write(*,*)'COAG M21ICT_2',q(i,k,lsfrm),tmpc,deltat
        end if
...
!        write(*,*)'ICTcoagu2_pair' ,pair_option_acoag
...
!        write(*,*)'ICTcoagu3_pair' ,pair_option_acoag
...
!
	...
	do ipair = 1, npair_acoag
	  ...
	  if ((m > 0) .and. (m <= ntot_amode)) then
	    l = numptr_amode(m)
	    if ((l > 0) .and. (l <= pcnst)) dotend(l) = .true.
            l = lsfcptr_amode(m)                                 !ICTsai
            if ((l > 0) .and. (l <= pcnst)) dotend(l) = .true.  !ICTsai
	  end if
	...
	do l = 1, pcnst
	    ...
	    do m = 1, ntot_amode
	        ...
	        if (l == lsfcptr_amode(m)) unit = 'm2/m2/s' !ICTsai
	    end do
...
!       lunerr, lunout = logical unit for error or diagnostic output
!
! modify input argument to SI unit 
!       dgni, dgnj = DRY median diameters for number distribution (m)
!       alnsgi, alnsgj = ln of geometric standard deviations (dimensionless)
!       rhopi, rhopj = WET density (kg/m3)
!       wetddrydi, wetddrydj = (WET median diameter)/(DRY median diameter)
!       temp = air temperature (K)
!       presscgs = air pressure (pa)
!       lunerr, lunout = logical unit for error or diagnostic output
!
!   output arguments
!       iok = status flag  (+1 = success, 0/negative = failure)
...
!   local variables
        real(r8) airprs, airtemp
        real(r8) dgacc, dgatk, pdensac, pdensat
        real(r8) batat(2), batac(2), bacat(2), bacac(2), c3ij
        real(r8) dp2bar_mks_i, dp2bar_mks_j, dp3bar_mks_i
        real(r8) wetddrydi, wetddrydj

!   check for reasonable inputs
        iok = -1
        if ((dgni .lt. 1.e-10) .or. (dgni .gt. 1.)) then
            write(*,9100) 'dgni', dgni
            return
        else if ((dgnj .lt. 1.e-10) .or. (dgnj .gt. 1.)) then
            write(*,9100) 'dgnj', dgnj
            return
!       else if (dgni .gt. dgnj) then
!           write(lunerr,9110) dgni, dgnj
!           return
            ...
            write(*,9100) 'alnsgj', alnsgj
            return
        else if ((rhopi .lt. 0.01) .or. (rhopi .gt. 3000.)) then
            write(*,9100) 'rhopi', rhopi
            return
        else if ((rhopj .lt. 0.01) .or. (rhopj .gt. 3000.)) then
            write(*,9100) 'rhopj', rhopj
            return
        else if ((temp .lt. 10.) .or. (temp .gt. 370.)) then
            write(*,9100) 'temp', temp
...
9100    format( '*** subr. calc_coag_snap - bad value for ', a,   &
                ' = ', 1pe15.5 )
!9110   format( '*** subr. calc_coag_coef_4 - dgni > dgnj -- ',
!     +         2(1pe15.5) )
        iok = +1
...
!   call interace to binkowski routines
!
!        write(*,'(A10,10e13.5)')'before cg',dgatk, dgacc, alnsgi, alnsgj, pdensat, &
!          pdensac,wetddrydi, wetddrydj,airtemp, airprs,  iok
        call snap_coag_rates(   &
          dgatk, dgacc, alnsgi, alnsgj, pdensat, pdensac,   &
          wetddrydi, wetddrydj,   &
          airtemp, airprs,  iok,   &
          batat, batac, bacat, bacac, c3ij )
!       write(*,'(A10,10e13.5)')'PAP_COAGU ',batat, batac, bacat, bacac, c3ij
...
!        betaii0 = batat(1) * 1.0e-14
!        betajj0 = bacac(1) * 1.0e-14
! unit transformation by ICTsai
        betaii0 = batat(1) * 1.0e-20
        betajj0 = bacac(1) * 1.0e-20
...
!        dp2bar_mks_i = (dgni**2) * exp( 2.0*alnsgi*alnsgi ) * 1.0e-4
!        dp3bar_mks_i = (dgni**3) * exp( 4.5*alnsgi*alnsgi ) * 1.0e-6
! convert unit by ICTsai
        dp2bar_mks_j = (dgnj**2) * exp( 2.0*alnsgj*alnsgj ) 
        dp2bar_mks_i = (dgni**2) * exp( 2.0*alnsgi*alnsgi ) 
        dp3bar_mks_i = (dgni**3) * exp( 4.5*alnsgi*alnsgi ) 

        betaii2 = (batat(2) / dp2bar_mks_i) * 1.0e-14
        betajj2 = (bacac(2) / dp2bar_mks_j) * 1.0e-14
! unit transfer by ICTsai
!        betaii2 = (batat(2) / dp2bar_mks_i) * 1.0e-20
!        betajj2 = (bacac(2) / dp2bar_mks_j) * 1.0e-20
!
!   modei-modej coagulation, number
!       dNi/dt = - betaij0*Ni*Nj
!       dNj/dt = 0.
...
!
!        betaij0  = batac(1) * 1.0e-14
        betaij0  = batac(1) * 1.0e-20 !ICTsai
...
!
        betaij2i = (batac(2) / dp2bar_mks_i) * 1.0e-14
        betaij2j = (bacat(2) / dp2bar_mks_i) * 1.0e-14
!        betaij2i = (batac(2) / dp2bar_mks_i) * 1.0e-20  !by ICTsai
!        betaij2j = (bacat(2) / dp2bar_mks_i) * 1.0e-20  !by ICTsai
...
!
!        betaij3  = ( c3ij / dp3bar_mks_i ) * 1.0e-14
        betaij3  = ( c3ij / dp3bar_mks_i ) * 1.0e-20 !by ICTsai

        return
        end subroutine calc_coag_snap

!-----------------------------------------------------------------------
        subroutine snap_coag_rates(   &
          dgatk, dgacc, xxlsgat, xxlsgac, pdensat, pdensac,   &
          wetddrydat, wetddrydac,   &
          airtemp, airprs,  iok,   &
          batat, batac, bacat, bacac, c3ij )
...
!   arguments
        integer  iok
        real(r8) dgatk, dgacc, xxlsgat, xxlsgac, pdensat, pdensac,   &
          wetddrydat, wetddrydac,   &
          airtemp, airprs
        real(r8) batat(2), batac(2), bacat(2), bacac(2), c3ij
...
!   local variables
      ...
      real(r8)    avo      ! avogadro's constant [ 1/mol ]
      parameter ( avo = 6.0221367e23_r8 )

      real(r8)    rgasuniv ! universal gas constant [ j/mol-k ]
      parameter ( rgasuniv = 8.314510_r8)

      real(r8)    boltz    ! boltzmann's constant [ j / k]
      parameter ( boltz = rgasuniv / avo )

      real(r8)    p0       !  starting standard surface pressure [ pa ]
      parameter ( p0 = 101325.0_r8 )

      real(r8)    t0       !  starting standard surface temperature [ k ]
      parameter ( t0 = 288.15_r8 )

      real(r8) xlm    ! atmospheric mean free path [ m ]
      real(r8) amu    ! atmospheric dynamic viscosity [ kg/m s ]

      real(r8) kfm, knc, lamda, sqrt_temp
...
! *** aitken - aitken mode coagulation
      ...
      call  intracoag_snap(airtemp,airprs,lamda,   &
                         kfm, knc,   &
                         dgatk,   &
                         xxlsgat,   &
                         wetddrydat,   &
...
! *** aitken accumulation mode coagulation
      ...
      kfm  = sqrt( 6.0_r8 * boltz * airtemp  /   &
                             ( pdensat + pdensac ) )
      call  intercoag_snap(airtemp,airprs,lamda,   &
                         kfm, knc,   &
                         dgatk, dgacc ,   &
                         xxlsgat, xxlsgac ,   &
                         wetddrydat, wetddrydac,   &
...
!     loss =  c30atac / cblk( vat3 )
      ...
      return
      end subroutine snap_coag_rates

! ---------------------------------------------------------------------
! fsb subrs to do gauss-hermite numerical quadrature

      subroutine intracoag_snap( airtp,airps,lamda, kfm, knc,   &
           dg, xlnsig, wetddryd,   &
           quads11, quadn11)

! modified witg SNAP by ICTsai
...
!
      ...
      real(r8) lamda ! mean free path
      real(r8) kfm, knc
      real(r8) dg, xlnsig, wetddryd
      real(r8) quads11, quadn11
      real(r8) pi
      ...
      real(r8) sum1sfm, sum2sfm, sum1nfm, sum2nfm
      real(r8) sum1snc, sum2snc, sum1nnc, sum2nnc
      real(r8) xi, wxi, xf, dp1p,dp1m,dp1psq,dp1msq, dp1pwet, dp1mwet
      real(r8) v1p,v1m, a2p,a2m,v2p,v2m
      real(r8) yi,wyi,yf,dp2p,dp2m,dp2psq,dp2msq, dp2pwet, dp2mwet
      real(r8) dspp,dsmp,dspm, dsmm
      real(r8) bppfm,bmpfm,bpmfm,bmmfm
      real(r8) bppnc,bmpnc,bpmnc,bmmnc
      real(r8) xx1, xx2
      real(r8) xbsfm, xbsnc, xbnfm, xbnnc
      real(r8) betafm, betanc
! used in SNAP
      real(r8)     airtp,airps
      real(r8) rg,rbb,ntot1,atot1,mtot1
      real(r8) paps11,papn11
      real(r8) kbmean

      real(r8)   a               ! approx cunningham corr. factor
      parameter( a = 1.246d0 )
...
! *** has a fixed number of gauss-herimite abscissas (n)
      integer n    ! one-half the number of abscissas
      parameter ( n = 5 )
      real(r8), save :: ghxi(n) ! gauss-hermite abscissas
      real(r8), save :: ghwi(n) ! gauss-hermite weights
...
!    breaks in number to facilitate comparison with printed table
       ...
       rg = dg/2.0_r8
       rbb=rg*1.5_r8
       mtot1=(rg**3)*exp(4.5_r8*xlnsig*xlnsig)
       atot1=(rg**2)*exp(2._r8*xlnsig*xlnsig)
       kbmean=kbm(rg,xlnsig,airtp,airps,lamda)
       call intra_snap(rg,xlnsig,ntot1,paps11,papn11)
! *** quads11 is the intra-modal coagulation term for 2nd moment
! *** quadn11 is the intra-modal coagulation term for number
       quadn11=kbmean*papn11
       quads11=4._r8*quadn11*paps11
!       write(*,'(A5,5e13.5)')'intra',dg,quads11,quadn11,kbmean,
!     &   rg,xlnsig


      return
      end subroutine intracoag_snap
!cc---calculate mean Brownian coagulation kernel
      function kbm(rr1,nxlnsig,airtp,airps,lamda)

      real(r8) cpi,cboz
      real(r8) lamda
      real(r8) kbm
      real(r8) airtp,airps,ndg,nxlnsig
      real(r8) rr1,rr2
      real(r8) zeda,kuna1,kuna2,cun1,cun2,difp1,difp2,gp1,gp2,velp1,velp2
      real(r8) amfp1,amfp2,mp1,mp2,denp
      parameter(cpi=3.141596_r8,cboz=1.3807E-23_r8,denp=1000.0_r8)
      real(r8) tmpbro1,tmpbro2,tmpbro3

      rr2=rr1*(1._r8+nxlnsig)
!      rr2=rr1*(1.5)

      zeda=1.8325E-5_r8*(416.16_r8/(airtp+120.0_r8))*((airtp/296.16_r8)**1.5)
!      (List,1984) dynamic viscosity of air(kg/m*s)
! thermal velocity of particles(m/s)

      kuna1=lamda/rr1
      kuna2=lamda/rr2

      mp1=(4.0_r8*cpi*(rr1**3.0)*denp)/3.0_r8
      mp2=(4.0_r8*cpi*(rr2**3.0)*denp)/3.0_r8

      velp1=((8.0_r8*cboz*airtp)/(cpi*mp1))**0.5
      velp2=((8.0_r8*cboz*airtp)/(cpi*mp2))**0.5
! thermal velocity of particles(m/s)
      cun1=1.0_r8+kuna1*(1.249_r8+0.42_r8*EXP(-0.87_r8/kuna1))
      cun2=1.0_r8+kuna2*(1.249_r8+0.42_r8*EXP(-0.87_r8/kuna2))
! Cunningham slip-flow correction

      difp1=(cboz*airtp*cun1)/(6.0_r8*cpi*rr1*zeda)
      difp2=(cboz*airtp*cun2)/(6.0_r8*cpi*rr2*zeda)
! particle diffusion coefficient
      amfp1=(2.0_r8*difp1)/(cpi*velp1)
      amfp2=(2.0_r8*difp2)/(cpi*velp2)
! apparent free path of the aerosol
      gp1=(((2.0_r8*rr1+amfp1)**3-(4.0_r8*rr1**2+amfp1**2)**1.5)  &
                                  /(6.0_r8*rr1*amfp1))-(2.0_r8*rr1)
      gp2=(((2.0_r8*rr2+amfp2)**3-(4.0*rr2**2.+amfp2**2.)**1.5)  &
                                  /(6.0_r8*rr2*amfp2))-(2.0_r8*rr2)

      tmpbro1=4.0_r8*cpi*(rr1+rr2)*(difp1+difp2)
      tmpbro2=(rr1+rr2)/((rr1+rr2)+(gp1**2+gp2**2)**0.5)
      tmpbro3=(4.0_r8*(difp1+difp2))/(((velp1**2+velp2**2)**0.5)*(rr1+rr2))
!      write(*,*)'ICTsai coagu11',tmpbro1,tmpbro2,tmpbro3

      kbm=tmpbro1/(tmpbro2+tmpbro3)

      return
      end function kbm

       SUBROUTINE intra_snap(rg,xlnsig,ntot,paps11,papn11)

       real(r8) rg,xlnsig,ntot
       real(r8) paps11,papn11
       real(r8) mtot1,atot1


       mtot1=log((rg**3)*exp(4.5_r8*xlnsig*xlnsig))
       atot1=log((rg**2)*exp(2._r8*xlnsig*xlnsig))

       papn11=exp(-0.7870056_r8+0.020737365_r8*atot1**2- &
         0.0092122897_r8*mtot1**2)
       paps11=exp(-0.82422034_r8+3.3432559_r8*atot1-1.5620292_r8*mtot1)
! m-2
!       papn11=exp(281.61074+51.445258*atot1+2.9231882*atot1*atot1
!     &  +0.082869616*atot1**3+0.0012058159*atot1**4+
!     &  7.1758604e-6*atot1**5-5.292616*mtot1-0.11842381*mtot1**2
!     &  -0.00073447218*mtot1**3)
!       paps11=exp(-0.82422034+3.3432559*atot1-1.5620292*mtot1)

       return
       end subroutine intra_snap

! ---------------------------------------------------------------------
       subroutine intercoag_snap( airtp,airps,lamda, kfm, knc, dg1, dg2,   &
           xlnsig1, xlnsig2,   &
           wetddryd1, wetddryd2,   &
           quads12, quads21, quadn12, quadv12 )
...
!.......................................................................
      ...
      real(r8) lamda ! mean free path
      real(r8) kfm, knc
      real(r8) dg1, xlnsig1, dg2, xlnsig2
      real(r8) wetddryd1, wetddryd2
      real(r8) quads12, quads21, quadn12, quadv12
      real(r8) pi
      parameter( pi = 3.14159265358979_r8)
      real(r8) two3rds
      parameter( two3rds = 2.0d0 /  3.0d0 )
      real(r8) sqrt2
      parameter(sqrt2 = 1.41421356237309_r8 )
      real(r8) sum1s12fm, sum1s21fm, sum2s12fm, sum2s21fm
      real(r8) sum1nfm, sum2nfm
      real(r8) sum1vfm, sum2vfm
      real(r8) sum1s12nc, sum1s21nc, sum2s12nc, sum2s21nc
      real(r8) sum1nnc, sum2nnc
      real(r8) sum1vnc, sum2vnc
      real(r8) xi, wxi,xf, dp1p, dp1m, dp1psq, dp1msq, dp1pwet, dp1mwet
      real(r8) a1p, a1m, v1p, v1m
      real(r8) a2p, a2m, v2p, v2m
      real(r8) yi, wyi, yf, dp2p, dp2m, dp2psq, dp2msq, dp2pwet, dp2mwet
      real(r8) dspp, dsmp, dspm, dsmm
      real(r8) bppfm, bmpfm, bpmfm, bmmfm
      real(r8) bppnc, bmpnc, bpmnc, bmmnc
      real(r8) xx1, xx2
      real(r8) xbsfm, xbsnc, xbnfm, xbnnc, xbvfm, xbvnc
      real(r8) betafm, betanc
! used in SNAP
      real(r8) ntot1,atot1,mtot1,ntot2,atot2,mtot2,am1,am2
      real(r8) airtp,airps
      real(r8) rg1,rg2
!      real(r8) quads12, quads21, quadn12, quadv12
      real(r8) kbmean
...
!    breaks in number to facilitate comparison with printed table
       rg1 = 0.5_r8*dg1
       rg2 = 0.5_r8*dg2
       mtot1=(rg1**3)*exp(4.5_r8*xlnsig1*xlnsig1)
       atot1=(rg1**2)*exp(2._r8*xlnsig1*xlnsig1)
       am1=atot1/mtot1

       mtot2=(rg2**3)*exp(4.5_r8*xlnsig2*xlnsig2)
       atot2=(rg2**2)*exp(2._r8*xlnsig2*xlnsig2)
       am2=atot2/mtot2
       kbmean=kbm2(rg1,rg2,airtp,airps,lamda)

       quadn12 = 2._r8*kbmean*deln1(atot1,mtot1)
       quads12 = 4._r8*dela1(atot1,mtot1)*quadn12
!c update
       quads21 = 4._r8*kbmean*dela2(am1,am2)
       quadv12 = 8._r8*delm1(am1,am2)*quadn12



! *** quads12 is the intermodal coagulation coefficient for surface area
! *** quadn12 is the intermodal coagulation coefficient for number
! *** quadv12 is the intermodal coagulation coefficient for 3rd moment

      return
      end subroutine intercoag_snap
!cc---calculate mean Brownian coagulation kernel
      function kbm2(rr1,rr2,airtp,airps,lamda)
      ...
      real(r8) zeda,kuna1,kuna2,cun1,cun2,difp1,difp2,gp1,gp2,velp1,velp2
      real(r8) amfp1,amfp2,mp1,mp2,denp
      parameter(cpi=3.141596_r8,cboz=1.3807E-23,denp=1000.0_r8)
      real(r8) tmpbro1,tmpbro2,tmpbro3
...
! apparent free path of the aerosol
      gp1=(((2.0_r8*rr1+amfp1)**3-(4.0_r8*rr1**2+amfp1**2)**1.5) &
                                 /(6.0_r8*rr1*amfp1))-(2.0*rr1)
      gp2=(((2.0_r8*rr2+amfp2)**3-(4.0_r8*rr2**2+amfp2**2)**1.5) &
                                 /(6.0_r8*rr2*amfp2))-(2.0_r8*rr2)

      tmpbro1=4.0_r8*cpi*(rr1+rr2)*(difp1+difp2)
      tmpbro2=(rr1+rr2)/((rr1+rr2)+(gp1**2+gp2**2)**0.5)
      tmpbro3=(4.0_r8*(difp1+difp2))/(((velp1**2+velp2**2)**0.5)*(rr1+rr2))
!       write(*,*)'ICTsai coagu22',tmpbro1,tmpbro2,tmpbro3

      kbm2=tmpbro1/(tmpbro2+tmpbro3)

      return
      end function kbm2

! functions of intercoagulation
!cc----calculate dM0/dt/N/N of nuclei mode
      function deln1(am1,am2)
...
!cc09/23/2011-----------------------------------------------------------
!      deln1=exp(-0.8371093-1.5934525*lnam1+1.0562354*lnam2)
!      deln1=exp(-0.96612765-1.7127088*lnam1+1.1342425*lnam2)
      deln1=exp(100.31943_r8+33.064865_r8*lnam1+0.95012952_r8*lnam1*lnam1+ &
        0.0086164823_r8*lnam1*lnam1*lnam1-16.622757_r8*lnam2-0.32564428_r8* &
        lnam2*lnam2-0.0019815024_r8*lnam2*lnam2*lnam2)


      return
      end function deln1

!cc----calculate dM2/dt of nuclei mode
      function dela1(am1,am2)
...
!      dela1=exp(0.059539022+4.3706108*lnam1-2.2450719*lnam2)
      dela1=exp(0.6302594_r8+4.4785933_r8*lnam1-2.3069902_r8*lnam2)

      return
      end function dela1
!cc----calculate dM2/dt of accumulation mode
      function dela2(am1,am2)
...
!cc09/23/2011-----------------------------------------------------------
      ...
      dela2=exp(-4.4175707_r8-2.8182922_r8*lnam1+0.97857013_r8*lnam2)

      return
      end function dela2

!cc----calculate dM3/dt of nuclei mode
      function delm1(am1,am2)
...
!      delm1=exp(-6.9290848-2.759002*lnam1+0.090368084*lnam2)
      delm1=exp(-5.7989668_r8-2.8032663_r8*lnam1+0.066207272_r8*lnam2)
