...
!add sgn by ICTsai
      call pbuf_add_field( 'SGNUM',      'global',  dtype_r8, (/pcols, pver, nmodes/),  sgnum_idx )
!      call pbuf_add_field( 'LNSGNUM',    'global',  dtype_r8, (/pcols, pver, nmodes/),  lnsgnum_idx )
!
   end if
...
subroutine aerosol_init(pbuf2d)
   ...
   if (clim_modal_aero .and. is_first_step()) then
      ...
      !ICTsai
      call pbuf_set_field(pbuf2d, sgnum_idx, 0.0_r8)
!      call pbuf_set_field(pbuf2d, lnsgnum_idx, 0.0_r8)
      !
   end if
...
!      print*,'prog ICTsai',prog_modal_aero
      if (prog_modal_aero) then
