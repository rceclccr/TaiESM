...
!   real(r8), intent(out) :: dgncur_awet(:,:,:)
   
! add for req calculation by ICTsai
   real(r8) req1,req2,sig1,sig2
   real(r8) radcesm,wetcesm,sigamtemp, rhtemp
...
!  eq size based on Chen et al. (2013)   ICTsai       check hygro
             if (rh(i,k).lt.1.e-4_r8) then
             rhtemp=1.e-4_r8
             else 
             rhtemp=rh(i,k)
             endif
!            write(*,*)'water ICTsai', hygro(i,k,m),rh(i,k),dryrad(i,k,m)
            wetrad(i,k,m)=(1._r8+hygro(i,k,m)/(-1.02733_r8+1.02654_r8/rhtemp+&
             6.07891E-10_r8/dryrad(i,k,m)))**(1._r8/3._r8)
            wetrad(i,k,m)=dryrad(i,k,m)*wetrad(i,k,m)
            wetvol(i,k,m)=pi43*wetrad(i,k,m)*wetrad(i,k,m)*wetrad(i,k,m)
            wetvol(i,k,m) = max(wetvol(i,k,m),dryvol(i,k,m))
            wetrad(i,k,m)=max(wetrad(i,k,m),dryrad(i,k,m))
