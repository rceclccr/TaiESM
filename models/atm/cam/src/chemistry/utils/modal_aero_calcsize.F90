...
use modal_aero_data, only: ntot_amode, nspec_amode, &
                           ...
                           voltosfchi_amode, voltosfclo_amode,&
                           voltosfc_amode,voltosfc_amode,sigmaghi_amode,&
                           sigmaglo_amode,sigmag_amode
!turn on prognostic surface area by ICTsai
...
use modal_aero_data,  only: numptrcw_amode, mprognum_amode, qqcw_get_field, lmassptrcw_amode, &
           ...
           cnst_name_cw,&
!turn on prognostic surface area by ICTsai
           mprogsfc_amode,lsfcptr_amode,lsfcptrcw_amode  
...
integer :: dgnum_idx = -1
integer :: sgnum_idx = -1  !ICTsai
!integer :: lnsgnum_idx = -1  !ICTsai
...
subroutine modal_aero_calcsize_init()
   ...
   sgnum_idx = pbuf_get_index('SGNUM')    !ICTsai
!    write(*,*)'dgnum_idx ICTsai',dgnum_idx,sgnum_idx 
!   lnsgnum_idx = pbuf_get_index('LNSGNUM')    !ICTsai
...
!      if (mprogsfc_amode(n) <= 0) then  !ICTsai
...
!       write(*,*)'sfcsiz ICTsai',n,tmpnamea,trim(tmpnamea),jac,numptr_amode(n) &
!           ,numptrcw_amode(n)
         unit = '#/m2/s'
...
subroutine modal_aero_calcsize_sub(state, ptend, deltat, pbuf, do_adjust_in, &
   ...
   real(r8), pointer :: sgncur_a(:,:,:)
!   real(r8), pointer :: lnsgncur_a(:,:,:)
   ...
   integer  :: lsa,lnsa,lsc,lnsc  ! added by ICTsai
   integer  :: n, nacc, nait
   ...
   real(r8), pointer :: fldscw(:,:)          ! added by ICTsai
   real(r8) :: delnum_a2, delnum_c2            !  work variables
   real(r8) :: delnum_a3, delnum_c3, delnum_t3 !  work variables
   real(r8) :: delsfc_a2, delsfc_c2            !  work variables by ICTsai
   real(r8) :: delsfc_a3, delsfc_c3, delsfc_t3 !  work variables by ICTsai
   ...
   real(r8) :: v2syy, v2sxx, v2szz           ! voltosfclo/hi of current mode (added by ICTsai)
   real(r8) :: v2syyrl, v2sxxrl              ! relaxed voltosfclo/hi  (added by ICTsai)
   real(r8) :: xfercoef
   ...
   real(r8) :: xfercoef_sfc_acc2ait          !by ICTsai
   real(r8) :: xfercoef_sfc_ait2acc          !by ICTsai
   real(r8) :: xferfrac_sfc_acc2ait          !by ICTsai
   real(r8) :: xferfrac_sfc_ait2acc          !by ICTsai

   real(r8) :: xfertend, xfertend_num(2,2)
   real(r8) :: xfertend_sfc(2,2)             !by ICTsai
   ! add variables for sfc by ICTsai
   real(r8) :: sfc_a0, sfc_c0, sfc_t0        ! initial number (#/mol_air)
   real(r8) :: sfc_a1, sfc_c1                ! working number (#/mol_air)
   real(r8) :: sfc_a2, sfc_c2, sfc_t2        ! working number (#/mol_air)
   real(r8) :: sfc_a, sfc_c, sfc_t           ! final number (#/mol_air)
   real(r8) :: sfcbnd                        ! bounded number
   real(r8) :: sfc_t_noxf
   real(r8) :: sfc_a_acc, sfc_c_acc
   real(r8) :: sfc_a_accsv(pcols,pver), sfc_c_accsv(pcols,pver)
   real(r8) :: sfc_a_aitsv(pcols,pver), sfc_c_aitsv(pcols,pver)
   real(r8) :: sfc_a_sv(pcols,pver,ntot_amode), sfc_c_sv(pcols,pver,ntot_amode)
   real(r8) :: v2scur_a(pcols,pver,ntot_amode)
   real(r8) :: v2scur_c(pcols,pver,ntot_amode)
!   real(r8) :: sgncur_a(pcols,pver,ntot_amode)
   real(r8) :: sgncur_c(pcols,pver,ntot_amode)
   real(r8) :: lnsgncur_a(pcols,pver,ntot_amode)
   real(r8) :: lnsgncur_c(pcols,pver,ntot_amode)
   real(r8) :: sgnyy, sgnxx                  ! sglo/hi of current mode
   real(r8) :: sg_a, sg_b                 ! sglo/hi of current mode
   ...
   call pbuf_get_field(pbuf, sgnum_idx, sgncur_a)   !ICTsai
!   call pbuf_get_field(pbuf, lnsgnum_idx, lnsgncur_a)   !ICTsai
   ...
   do n = 1, ntot_amode
      ...
      do k=top_lev,pver
        ...
        ! add sfc by ICTsai
         if (mprogsfc_amode(n) > 0) then
               v2scur_a(i,k,n) = voltosfc_amode(n)
               v2scur_c(i,k,n) = voltosfc_amode(n)
               sgncur_a(i,k,n) = sigmag_amode(n)
               sgncur_c(i,k,n) = sigmag_amode(n)
...
! add sfc by ICTsai
         if (mprogsfc_amode(n) > 0) then
!         lnsa = lsfcptr_amode(n)-loffset
!         lnsc = lsfcptrcw_amode(n) - loffset
         lnsa = lsfcptr_amode(n)
         lnsc = lsfcptrcw_amode(n) 
         fldscw => qqcw_get_field(pbuf,lsfcptrcw_amode(n),lchnk,.true.)   ! added by ICTsai
         end if
!
...
! add v2sfc conversion by ICTsai
         if (mprogsfc_amode(n) > 0) then
         v2sxx = voltosfchi_amode(n)
         v2syy = voltosfclo_amode(n)
         v2sxxrl = v2sxx/frelaxadj
         v2syyrl = v2syy*frelaxadj
         sgnxx = sigmaghi_amode(n)
         sgnyy = sigmaglo_amode(n)
         end if
!
...
! add v2sfc conversion by ICTsai
           if (mprogsfc_amode(n) > 0) then
           if (n == nait) v2sxx = v2sxx/1.0e6
           if (n == nacc) v2syy = v2syy*1.0e6
            v2sxxrl = v2sxx/frelaxadj   ! NEW
            v2syyrl = v2syy*frelaxadj   ! NEW
           end if
!
...
! added sfc by ICTsai
         if (mprogsfc_amode(n) > 0) then
               sfc_a0 = q(i,k,lnsa)
               sfc_a = max( 0.0_r8, sfc_a0 )
               sfc_c0 = fldscw(i,k)
               sfc_c = max( 0.0_r8, sfc_c0 )
          endif
!
            ...
            if ( do_adjust) then
               ...
               if ((drv_a <= 0.0_r8) .and. (drv_c <= 0.0_r8)) then
                ...
                ! adjust sfc, added by ICTsai
                    if (mprogsfc_amode(n) > 0) then
                     sfc_a =  0.0_r8
                     dqdt(i,k,lnsa) = -sfc_a0*deltatinv
                     sfc_c = 0.0_r8
                     dqqcwdt(i,k,lnsc) = -sfc_c0*deltatinv
                    end if
                 !
                  ...
                  ! adjust sfc, added by ICTsai
                    if (mprogsfc_amode(n) > 0) then
                     ...
                     dqqcwdt(i,k,lnsc) = -sfc_c0*deltatinv
                     sfc_a1 = sfc_a
                     sfcbnd = max( drv_a*v2sxx, min( drv_a*v2syy, sfc_a1 ) )
                     sfc_a  = sfc_a1 + (sfcbnd - sfc_a1)*fracadj
                     dqdt(i,k,lnsa) = (sfc_a - sfc_a0)*deltatinv
                    end if
                  !
                 ...
                 ! adjust sfc, added by ICTsai
                    if (mprogsfc_amode(n) > 0) then
                     ...
                     dqdt(i,k,lnsa) = -sfc_a0*deltatinv
                     sfc_c1 = sfc_c
                     sfcbnd = max( drv_c*v2sxx, min( drv_c*v2syy, sfc_c1 ) )
                     sfc_c  = sfc_c1 + (sfcbnd - sfc_c1)*fracadj
                     dqqcwdt(i,k,lnsc) = (sfc_c - sfc_c0)*deltatinv
                    end if
                   !
               else
                  ...
                  if ((delnum_a2 == 0.0_r8) .and. (delnum_c2 /= 0.0_r8)) then
                     ...
                     num_c2 = max( drv_c*v2nxxrl, min( drv_c*v2nyyrl,   &
                       ...
                       ! adjust sfc, added by ICTsai
                       if (mprogsfc_amode(n) > 0) then
                     sfcbnd = max( drv_a*v2sxxrl, min( drv_a*v2syyrl, sfc_a1 ) )
                     delsfc_a2 = (sfcbnd - sfc_a1)*fracadj
                     sfc_a2 = sfc_a1 + delsfc_a2
                     sfcbnd = max( drv_c*v2sxxrl, min( drv_c*v2syyrl, sfc_c1 ) )
                     delsfc_c2 = (sfcbnd - sfc_c1)*fracadj
                     sfc_c2 = sfc_c1 + delsfc_c2
                     if ((delsfc_a2 == 0.0) .and. (delsfc_c2 /= 0.0)) then
                        sfc_a2 = max( drv_a*v2sxxrl, min( drv_a*v2syyrl,   &
                             sfc_a1-delsfc_c2 ) )
                     else if ((delsfc_a2 /= 0.0) .and. (delsfc_c2 == 0.0)) then
                        sfc_c2 = max( drv_c*v2sxxrl, min( drv_c*v2syyrl,   &
                             sfc_c1-delsfc_a2 ) )
                     end if
                       end if
                      !
                  ...
                  dqqcwdt(i,k,lnc) = (num_c - num_c0)*deltatinv
                     ! adjust sfc, added by ICTsai
                      if (mprogsfc_amode(n) > 0) then
                       sfc_t2 = sfc_a2 + sfc_c2
                       delsfc_a3 = 0.0_r8
                       delsfc_c3 = 0.0_r8
                    if (sfc_t2 < drv_t*v2sxx) then
                        delsfc_t3 = (drv_t*v2sxx - sfc_t2)*fracadj
                        ! if you are here then (sfc_a2 < drv_a*v2sxx) and/or
                        !                      (sfc_c2 < drv_c*v2sxx) must be true
                        if ((sfc_a2 < drv_a*v2sxx) .and. (sfc_c2 < drv_c*v2sxx)) then
                           delsfc_a3 = delsfc_t3*(sfc_a2/sfc_t2)
                           delsfc_c3 = delsfc_t3*(sfc_c2/sfc_t2)
                        else if (sfc_c2 < drv_c*v2sxx) then
                           delsfc_c3 = delsfc_t3
                        else if (sfc_a2 < drv_a*v2sxx) then
                           delsfc_a3 = delsfc_t3
                        end if
                     else if (sfc_t2 > drv_t*v2syy) then
                        delsfc_t3 = (drv_t*v2syy - sfc_t2)*fracadj
                        ! if you are here then (sfc_a2 > drv_a*v2syy) and/or
                        !                      (sfc_c2 > drv_c*v2syy) must be true
                        if ((sfc_a2 > drv_a*v2syy) .and. (sfc_c2 > drv_c*v2syy)) then
                           delsfc_a3 = delsfc_t3*(sfc_a2/sfc_t2)
                           delsfc_c3 = delsfc_t3*(sfc_c2/sfc_t2)
                        else if (sfc_c2 > drv_c*v2syy) then
                           delsfc_c3 = delsfc_t3
                        else if (sfc_a2 > drv_a*v2syy) then
                           delsfc_a3 = delsfc_t3
                        end if
                     end if
                     dqdt(i,k,lnsa) = (sfc_a - sfc_a0)*deltatinv
                     sfc_c = sfc_c2 + delsfc_c3
                     dqqcwdt(i,k,lnsc) = (sfc_c - sfc_c0)*deltatinv
            ...
            if (drv_a > 0.0_r8) then
               if (num_a <= drv_a*v2nxx) then
                 ...
                 if (mprogsfc_amode(n) > 0) then
                  v2scur_a(i,k,n) = v2sxx   !ICTsai
                  sgncur_a(i,k,n) = sgnxx    !ICTsai
                 endif
                ...
                if (mprogsfc_amode(n) > 0) then
                  v2scur_a(i,k,n) = v2syy   !ICTsai
                  sgncur_a(i,k,n) = sgnyy   !ICTsai
                 endif

               else
                  dgncur_a(i,k,n) = (drv_a/(dumfac*num_a))**third
                  v2ncur_a(i,k,n) = num_a/drv_a
                    ! calculate sigma , added by ICTsai
                  if (mprogsfc_amode(n) > 0) then
                     v2scur_a(i,k,n) = sfc_a/drv_a
                     sg_a = log(drv_a/(dgncur_a(i,k,n)**3*num_a))*2._r8/9._r8
                     sg_b = sqrt(sg_a)
                     sgncur_a(i,k,n) = exp(sg_b)
                  end if !sfc
            ...
            qsrflx(i,lna,2,jac) = qsrflx(i,lna,2,jac) + min(0.0_r8,dqdt(i,k,lna))*pdel_fac
              ! calculate sfc,sigma , added by ICTsai
               if (mprogsfc_amode(n) > 0) then
               qsrflx(i,lnsa,1,jac) = qsrflx(i,lnsa,1,jac) + max(0.0_r8,dqdt(i,k,lnsa))*pdel_fac
               qsrflx(i,lnsa,2,jac) = qsrflx(i,lnsa,2,jac) + min(0.0_r8,dqdt(i,k,lnsa))*pdel_fac
               end if
              !
            ...
            if (drv_c > 0.0_r8) then
               if (num_c <= drv_c*v2nxx) then
                  ...
                  v2ncur_c(i,k,n) = num_c/drv_c
                    ! calculate sigma , added by ICTsai
                  if (mprogsfc_amode(n) > 0) then
                     v2scur_c(i,k,n) = sfc_c/drv_c
                     sg_a = log(drv_c/(dgncur_c(i,k,n)**3*num_c))*2._r8/9._r8
                     sg_b = sqrt(sg_a)
                     sgncur_c(i,k,n) = exp(sg_b)
                  end if !sfc
               end if
            end if
            jac = 2
            ...
            qsrflx(i,lnc,2,jac) = qsrflx(i,lnc,2,jac) + min(0.0_r8,dqqcwdt(i,k,lnc))*pdel_fac
              ! calculate sfc, sigma , added by ICTsai
                if (mprogsfc_amode(n) > 0) then
               qsrflx(i,lnsc,1,jac) = qsrflx(i,lnsc,1,jac) + max(0.0_r8,dqqcwdt(i,k,lnsc))*pdel_fac
               qsrflx(i,lnsc,2,jac) = qsrflx(i,lnsc,2,jac) + min(0.0_r8,dqqcwdt(i,k,lnsc))*pdel_fac
                end if
            ...
            if ( do_aitacc_transfer ) then
               ...
               ! calculate sfc,sigma , added by ICTsai
                    if (mprogsfc_amode(n) > 0) then
                     sfc_a_aitsv(i,k) = sfc_a
                     sfc_c_aitsv(i,k) = sfc_c
                    endif
               !
               else if (n == nacc) then
               ...
               ! calculate sfc,sigma , added by ICTsai
                    if (mprogsfc_amode(n) > 0) then
                     sfc_a_accsv(i,k) = sfc_a
                     sfc_c_accsv(i,k) = sfc_c
                    endif
               !
               end if
            end if
            ...
            num_c_sv(i,k,n) = num_c
               ! calculate sfc,sigma , added by ICTsai
               if (mprogsfc_amode(n) > 0) then
               sfc_a_sv(i,k,n) = sfc_a
               sfc_c_sv(i,k,n) = sfc_c
               end if
               !
         end do
      end do
   ...
   if ( do_aitacc_transfer ) then
      ...
      v2szz = sqrt(voltosfc_amode(nait)*voltosfc_amode(nacc))  !ICTsai
      ...
      do  k = top_lev, pver
         do  i = 1, ncol
            ...
            pdel_fac = pdel(i,k)/gravit   ! = rho*dz
            xfertend_num(:,:) = 0.0_r8
            xfertend_sfc(:,:) = 0.0_r8    ! added by ICTsai
            ...
            num_t = num_a_aitsv(i,k) + num_c_aitsv(i,k)
               ! calculate sfc,sigma , added by ICTsai
              !write(*,*)'ICTsfc',mprogsfc_amode(1),mprogsfc_amode(2)
                if (mprogsfc_amode(2) > 0) then
                 sfc_t = sfc_a_aitsv(i,k) + sfc_c_aitsv(i,k)
                end if
               !
            if (drv_t > 0.0_r8) then
               if (num_t < drv_t*v2nzz) then
                  ...
                  if (num_t < drv_t*voltonumb_amode(nacc)) then
                     ...
                     xferfrac_sfc_ait2acc = 1.0_r8  !by ICTsai
                  else
              ...
              ! calculate sfc,sigma , added by ICTsai
                if (mprogsfc_amode(2) > 0) then
                       xferfrac_sfc_ait2acc = xferfrac_vol_ait2acc*   &
                             (drv_t*voltosfc_amode(nacc)/sfc_t)  !double check
                end if
              !
                ...
                if (mprogsfc_amode(2) > 0) then
                  xfercoef_sfc_ait2acc = xferfrac_sfc_ait2acc*tadjinv         !ICTsai
                  xfertend_sfc(1,1) = num_a_aitsv(i,k)*xfercoef_sfc_ait2acc   !by ICTsai
                  xfertend_sfc(1,2) = sfc_c_aitsv(i,k)*xfercoef_sfc_ait2acc   !by ICTsai
                end if
               end if
            end if
            ...
            xfercoef_sfc_acc2ait = 0.0_r8           !by ICTsai

            drv_t = drv_a_accsv(i,k) + drv_c_accsv(i,k)
            num_t = num_a_accsv(i,k) + num_c_accsv(i,k)
             ! calculate sfc,sigma , added by ICTsai
                if (mprogsfc_amode(1) > 0) then
                sfc_t =  sfc_a_accsv(i,k) + sfc_c_accsv(i,k)
                end if
             !
            ...
            if (drv_t > 0.0_r8) then
               if (num_t > drv_t*v2nzz) then
                  ...
                  drv_t = max( 0.0_r8, drv_t - drv_t_noxf )
                       ! calculate sfc,sigma , added by ICTsai
                      if (mprogsfc_amode(1) > 0) then
                        sfc_t_noxf = drv_t_noxf*voltosfclo_amode(nacc)
                        sfc_t0 = sfc_t
                        sfc_t = max( 0.0_r8, sfc_t - sfc_t_noxf )
                      end if
                       !
               end if
            end if
            ...
            if (drv_t > 0.0_r8) then
               if (num_t > drv_t*v2nzz) then
                  ...
                  if (num_t > drv_t*voltonumb_amode(nait)) then
                     ...
                     xferfrac_vol_acc2ait = 1.0_r8
                      if (mprogsfc_amode(1) > 0) then
                     xferfrac_sfc_acc2ait = 1.0_r8                 !by ICTsai
                      end if
                  else
                     ...
                     xferfrac_num_acc2ait = xferfrac_vol_acc2ait*   &
                      ...
                      ! calculate sfc,sigma , added by ICTsai
                      if (mprogsfc_amode(1) > 0) then
                        xferfrac_sfc_acc2ait = xferfrac_vol_acc2ait*   &
                             (drv_t*voltosfc_amode(nait)/sfc_t)
                      end if
                       !
                     ...
                     if ((xferfrac_num_acc2ait <= 0.0_r8) .or.   &
                      ...
                      if (mprogsfc_amode(1) > 0) then
                        xferfrac_sfc_acc2ait = 0.0_r8            !by ICTsai
                      end if
             ...
             ! calculate sfc,sigma , added by ICTsai
                if (mprogsfc_amode(1) > 0) then
                  xfercoef_sfc_acc2ait = xferfrac_sfc_acc2ait*tadjinv  !by ICTsai
                     xfertend_sfc(2,1) = sfc_a_accsv(i,k)*xfercoef_sfc_acc2ait
                     xfertend_sfc(2,2) = sfc_c_accsv(i,k)*xfercoef_sfc_acc2ait
                end if
             !
               end if
            end if
            ...
            if (ixfer_ait2acc+ixfer_acc2ait > 0) then
               ...
               do n = nait, nacc, (nacc-nait)
                  if (n .eq. nait) then
                    ...
                    ! calculate sfc,sigma , added by ICTsai
                    if (mprogsfc_amode(1) > 0) then
                        duma = (xfertend_sfc(1,1) - xfertend_sfc(2,1))*deltat
                        sfc_a     = max( 0.0_r8, sfc_a_aitsv(i,k) - duma )
                        sfc_a_acc = max( 0.0_r8, sfc_a_accsv(i,k) + duma )
                        duma = (xfertend_sfc(1,2) - xfertend_sfc(2,2))*deltat
                        sfc_c     = max( 0.0_r8, sfc_c_aitsv(i,k) - duma )
                        sfc_c_acc = max( 0.0_r8, sfc_c_accsv(i,k) + duma )
                    end if
                    !
                  else
                     num_a = num_a_acc
                     ...
                     drv_c = drv_c_acc
                    if (mprogsfc_amode(1) > 0) then
                     sfc_a = sfc_a_acc       !add by ICTsai
                     sfc_c = sfc_c_acc       !added by ICTsai
                    end if
                  end if
                  ...
                  if (drv_a > 0.0_r8) then
                     if (num_a <= drv_a*voltonumbhi_amode(n)) then
                        ...
                        v2scur_a(i,k,n) = voltosfchi_amode(n) !ICTsai
                        sgncur_a(i,k,n)  = sigmaghi_amode(n)  !ICTsai

                     else if (num_a >= drv_a*voltonumblo_amode(n)) then
                        dgncur_a(i,k,n) = dgnumlo_amode(n)
                        v2ncur_a(i,k,n) = voltonumblo_amode(n)
                        v2scur_a(i,k,n) = voltosfclo_amode(n) !ICTsai
                        sgncur_a(i,k,n)  = sigmaglo_amode(n)  !ICTsai
                     ...
                     dgncur_a(i,k,n) = dgnum_amode(n)
                     v2ncur_a(i,k,n) = voltonumb_amode(n)
                     v2scur_a(i,k,n) = voltosfc_amode(n)   !ICTsai
                     sgncur_a(i,k,n)  = sigmag_amode(n)    !ICTsai
                  ...
                  if (drv_c > 0.0_r8) then
                     if (num_c <= drv_c*voltonumbhi_amode(n)) then
                        ...
                        v2scur_c(i,k,n) = voltosfchi_amode(n)  !ICTsai
                        sgncur_c(i,k,n)  = sigmaghi_amode(n)   !ICTsai

                     else if (num_c >= drv_c*voltonumblo_amode(n)) then
                        dgncur_c(i,k,n) = dgnumlo_amode(n)
                        v2ncur_c(i,k,n) = voltonumblo_amode(n)
                        v2scur_c(i,k,n) = voltosfclo_amode(n) !ICTsai
                        sgncur_c(i,k,n)  = sigmaglo_amode(n)  !ICTsai
                     ...
                     dgncur_c(i,k,n) = dgnum_amode(n)
                     v2ncur_c(i,k,n) = voltonumb_amode(n)
                     v2scur_c(i,k,n) = voltosfc_amode(n)   !ICTsai
                     sgncur_c(i,k,n)  = sigmag_amode(n)    !ICTsai
   ...
   do n = 1, ntot_amode 
     ...
     ! output dgn and sigma by ICTsai
!       if ( history_aerosol ) then
      if (mprogsfc_amode(1) > 0) then

                 write( tmpnamea, '(i3.3)' ) n
!                call outfld('dgnd_a'//tmpnamea(3:3), dgncur_a(:,:,n), pcols, lchnk)
                call outfld('sgnd_a'//tmpnamea(3:3), sgncur_a(:,:,n), pcols, lchnk)
      end if
!           write(*,*)'ICT outsgn',n,'dgncur_a'//trnum(3:3),'sgncur_a'//trnum(3:3)
!       end if
...
subroutine modal_aero_calcsize_diag(state, pbuf, list_idx_in, dgnum_m, & 
   ...
   real(r8), optional, pointer      :: sgnum_m(:,:,:) ! interstital aerosol dry number mode radius (m)
   ...
   real(r8), pointer :: sgncur_a(:,:) ! (pcols,pver)  !ICTsai
   ...
   if (list_idx /= 0) then
      ...
      allocate(sgnum_m(pcols,pver,nmodes), stat=stat)    !ICTsai
      if (stat > 0) then
         call endrun('modal_aero_calcsize_diag: allocation FAILURE: dgnum_m')
         call endrun('modal_aero_calcsize_diag: allocation FAILURE: sgnum_m')  !ICTsai
      end if
   end if
   ...
   do n = 1, nmodes
      ...
      !ICTsai
       if (list_idx == 0) then
         call pbuf_get_field(pbuf, sgnum_idx, sgncur_a, start=(/1,1,n/), kount=(/pcols,pver,1/))
      else
         sgncur_a => sgnum_m(:,:,n)
      end if
      ...
      ! get mode properties
      call rad_cnst_get_mode_props(list_idx, n, dgnum=dgnum, dgnumhi=dgnumhi, dgnumlo=dgnumlo, sgnum=sgnum)

      ! get mode number mixing ratio
      call rad_cnst_get_mode_num(list_idx, n, 'a', state, pbuf, mode_num)

      dgncur_a(:,:) = dgnum
      sgncur_a(:,:) = sgnum  !ICTsai
      dryvol_a(:,:) = 0.0_r8
