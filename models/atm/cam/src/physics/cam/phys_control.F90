...
!
! Revision history:
! 2014-11-27  Yi-Chi Wang,    modify the check for HP and HBGFS
...
character(len=32) :: cam_chempkg          = unset_str  ! CAM chemistry package [waccm_mozart | 
                                                       ...
                                                       !  waccm_mozart_mam3 | trop_snap | trop_snap_mix | none
...
subroutine phys_ctl_readnl(nlfile)
   ...
   ! +++ Yi-Chi : add HP to shallow scheme
   if (.not. (shallow_scheme .eq. 'Hack' .or. shallow_scheme .eq. 'UW' .or. shallow_scheme .eq. 'CLUBB_SGS' .or. &
              shallow_scheme .eq. 'HP')) then
   ...
   if (.not. (eddy_scheme .eq. 'HB' .or. eddy_scheme .eq. 'HBR' .or. eddy_scheme .eq. 'diag_TKE' .or. &
              eddy_scheme .eq. 'CLUBB_SGS' .or. eddy_scheme .eq. 'HBGFS') ) then
   !if (.not. (eddy_scheme .eq. 'HB' .or. eddy_scheme .eq. 'HBR' .or. eddy_scheme .eq. 'diag_TKE' .or. &
   ...
   ! +++ Yi-Chi : allow Hack to go with HBGFS
   if (( shallow_scheme .eq. 'Hack' ) .and. ( ( eddy_scheme .ne. 'HB' ) .and. ( eddy_scheme .ne. 'HBR' ) .and. ( eddy_scheme .ne. 'HBGFS' ))) then
   ...
   prog_modal_aero = (     cam_chempkg_is('trop_mam3') &
                      ...
                      .or. cam_chempkg_is('trop_snap') &   !ICTsai
                      .or. cam_chempkg_is('trop_snap_mix') )  !ICTsai
