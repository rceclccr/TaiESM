   ...
   ! --------------------------------------------
   ! Yi-Chi Wang : Dec 2014
   ! - the clean version of the code
   ! - debug for error message from check_energy 
   !   move rliq2 position, replace physical constants
   ! --------------------------------------------   !
   ! Yi-Chi Wang : Oct 2014
   ! - move the code to CAM5.3                      !
   !   mainly follows the structure of CAM5.3
   ! -----------------------------------------------!
   !cjshiu Mar 2012 modify to include shallow convection scheme of Han and Pan (2011)
   !cj following coding structures of UW
   !----------------------------------------------- !
...
10   continue
  ...
  ! cjshiu add initial for HP
  case('HP') ! Han and Pan (2011) scheme

! cjshiu need to confirm if we also need to limit shallow convection to regions below 40 mb

   if( masterproc ) then
       write(iulog,*) 'convect_shallow_init: HP shallow convection'
   endif
   ! Yi-Chi (March 2013) :
   ! - fix the bug for initializing this part.
   ! - Need to rewrite this part if with time.
   !call shalcnv_init( gravit , cpair , latvap ,                       &
   !                  4.6150e2_r8 , zvir , 2.7315e2_r8 ,                   &
   !                  rair , 1.8460e3_r8 , 4.1855e3_r8 ,                   &
   !                  rair/(4.6150e2_r8) , rair/(4.6150e2_r8)-1._r8 )
   call shalcnv_init( gravit , cpair , latvap ,                       &
                     tmelt   , zvir  , tmelt ,               &
                     rair    , cpwv  , cpliq ,                   &
                     rair/rh2o , (rair/rh2o)-1._r8)
   ! shalcnv_init (con_g , con_cp , con_hvap ,   &
   !                      con_rv, con_fvirt, con_t0c,   &
   !                      con_rd, con_cvap, con_cliq,   &
   !                      con_eps, con_epsm1)
!cjshiu set physcons used in GFS as initial values (those already in HK or UW are used to replace those of GFS)
  ! cj add END
...
!cjshiu add three additional parameters needed by HP2011
                                   landfrac, shflx, cflx,kcnv_hpin ) ! Yi-Chi:add flag for deep convection
!cjshiu add END
...
! cjshiu addend
   ...
   ! -----------------------
   ! I/O for Han and Pan 2011
   ! -----------------------
   ! Yi-Chi adds flag to stop shallow when deep is activated
   integer,             intent(in)    :: kcnv_hpin(pcols)
   !cjshiu add landfrac here
   real(r8),            intent(in)    :: landfrac(pcols)                 ! Land fraction
   real(r8),            intent(in)    :: shflx(pcols)                    ! sensible heat flux (W/m2)
   ...
   ! ------------------------!
   ! Variables for Han and Pan scheme
   ! ----------------------- !
   !cjshiu add initial values needed by H&P2011
   integer  :: jcap_hp, ncloud_hp
   real(r8) :: rcs_hp(pcols)
   integer  :: kcnv_hp(pcols)
   real(r8) :: slimsk_hp(pcols)
   ! --- YiChi
   ...
   case('HP')
      ...
      ! -------------------------------------- !
      lq(:) = .TRUE.
      call physics_ptend_init( ptend_loc, state%psetcols, 'hp_conv', ls=.true., lu=.true., lv=.true., lq=lq  ) ! Initialize local ptend type

      jcap_hp = 126
      !write(iulog,*)'YiChi(convect_shallow): kcnv_hpin:',kcnv_hpin
      kcnv_hp(:) = INT(kcnv_hpin(:))
      !write(iulog,*)'YiChi(convect_shallow): kcnv_hp:',kcnv_hp

      slimsk_hp(:) = 0._r8
      do i = 1, ncol
        rcs_hp(i)    = 1._r8
       if(landfrac(i).gt.0.5) then
        slimsk_hp(i) = 1._r8 ! for land
        else
        slimsk_hp(i) = 0._r8 ! for water
       endif
      enddo

       ncloud_hp = 1
       call  shalcnv_inv( lchnk         , ncol , pcols , pver  , pcnst  , jcap_hp ,        &
                         ztodt         , state%pdel   , state%pmid     , state%ps(:ncol),  &
                         state%zm      , state%q(:,:,ixcldliq) ,                           &
                         state%q(:,:,1), state%t      , state%u        , state%v ,         &
                         rcs_hp(:ncol) , precc(:ncol) , kcnv_hp(:ncol) , slimsk_hp(:ncol), &
                         state%omega   , ncloud_hp    , pblh(:ncol)   ,                   &
                         shflx(:ncol)  , cflx(:ncol,1),                                   &
                         ptend_loc%s   , ptend_loc%q  , ptend_loc%u    , ptend_loc%v ,    &
                         cmfmc2        , rprdsh       , cmfsl          , cmflq       ,    &
                         qc2           , icwmr        , rliq2 , cnb2(:ncol) , cnt2(:ncol) , evapcsh)

   ! Yi-Chi : currently, cmfsl and cmflq = 0
      cmfsl = 0.0
      cmflq = 0.0
   ! ---Yi-Chi
   !cj add END
   ! Yi-Chi : add momentum transport
      !ptend_loc%lu = .TRUE.
      !ptend_loc%lv = .TRUE.

!cjshiu add this as did of UW shallow scheme
 ...
 ! Modification : I should check whether below computation of freqsh is correct.
   ...
   !write(iulog,*)'YiChi(convect_shallow): t_pre_Cu:',state1%t
   !write(iulog,*)'YiChi(convect_shallow): qv_pre_Cu:',state1%q(:,:,1)
   !write(iulog,*)'YiChi(convect_shallow): u_pre_Cu:',state1%u
   !write(iulog,*)'YiChi(convect_shallow): v_pre_Cu:',state1%v
   ...
   !write(iulog,*)'YiChi(convect_shallow): t_aft_Cu:',state1%t
   !write(iulog,*)'YiChi(convect_shallow): qv_aft_Cu:',state1%q(:,:,1)
   !write(iulog,*)'YiChi(convect_shallow): u_aft_Cu:',state1%u
   !write(iulog,*)'YiChi(convect_shallow): v_aft_Cu:',state1%v
