...
!   Since the diffusivity includes forcing both from surface and cloud top,
!      I modified every diffusivity term associated with counter gradients as only
...
!
  ...
  use diffusion_solver, only : vd_lu_solve, vd_lu_decomp, vdiff_selector, vdiff_select, &
                               operator(.not.), any, new_fieldlist_vdiff, diffuse
  !use diffusion_solver, only : vd_lu_solve, vd_lu_decomp, vdiff_selector, vdiff_select, &
  !                             operator(.not.), any, new_fieldlist_vdiff
  ! +++ Yi-Chi
  !use physconst,          only: rair, cpair, gravit, zvir, latvap, karman
  ! ---
  ...
  ! +++ Yi-Chi : add one subroutine for GFSHB so UW scheme can keep intact
  !public compute_vdiff
  public hbgfs_compute_vdiff
  public init_hbgfs_vdiff
  ! --- Yi-Chi
  ...
  subroutine init_hbgfs_vdiff( kind, rair_in, gravit_in, do_iss_in, errstring )
  ...
  end subroutine init_hbgfs_vdiff
...
! +++ Yi-Chi +++
  subroutine hbgfs_compute_vdiff( lchnk           ,                                                             &
...
!joanne add dktx,dkux (for counter gradient) to separate surface diffusivity (dktx,dkux) from KVM,KVH
                            do_molec_diff   , dktx, dkux , compute_molec_diff , vd_lu_qdecomp,kvt)
!joanne
!                            do_molec_diff  , compute_molec_diff, vd_lu_qdecomp, kvt ) ! default
!joanne
...
!!joanne add dktx,dkux (for counter gradient) to separate surface diffusivity (dktx,dkux) from KVM,KVH
    real(r8) :: dkux(pcols,pver+1)       ! GFS
    real(r8) :: dktx(pcols,pver+1)       ! GFS
!joanne
...
!!joanne modified the diffusivity associated with counter gradient
           dse(:ncol,k) = dse(:ncol,k) + ztodt * rpdel(:ncol,k) * gravit  *                &
                                       ( rhoi(:ncol,k+1) * dktx(:ncol,k+1) * cgh(:ncol,k+1) &
                                       - rhoi(:ncol,k  ) * dktx(:ncol,k  ) * cgh(:ncol,k  ) )
      !     dse(:ncol,k) = dse(:ncol,k) + ztodt * rpdel(:ncol,k) * gravit  *                &
...
!!joanne modified the diffusivity associated with counter gradient
              q(:ncol,k,m) = q(:ncol,k,m) + &
                           ...
                           ( rhoi(:ncol,k+1) * dktx(:ncol,k+1) * cgs(:ncol,k+1)                    &
                           - rhoi(:ncol,k  ) * dktx(:ncol,k  ) * cgs(:ncol,k  ) )
!
!              q(:ncol,k,m) = q(:ncol,k,m) + &
!                             ztodt * rpdel(:ncol,k) * gravit  * ( cflx(:ncol,m) * rrho(:ncol) ) * &
...
!!joanne
    ...
    return
  end subroutine hbgfs_compute_vdiff

! +++ Yi-Chi : remove diffuse function
...
! --------------------------------------------------- !
! +++ Yi-Chi : Nov 2014
! remove all repetitive function as diffusion solver
! need to make sure if they are alright.
! ---------------------------------------------------- !


end module hbgfs_diffusion_solver
