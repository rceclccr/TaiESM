...
!cjshiu add namelist for Uniform PDF cloud fraction
...
!cjshiu add pbuf registration here to check if it works if not directly move uniform pdf pbuf parameters to here
   use cldwat2m_macro_updf, only : uniform_pdf_register
!cjshiu end add   
!Yi-Chi add
   use cldwat2m_macro_tpdf, only : trig_pdf_register
! Yi-Chi end add
! ------Yi-Chi end add --------------!
  ...
  !cjshiu add macrophysical namelist for U_pdf
  call phys_getopts(macrop_scheme_out=macrop_scheme)
!cjshiu add macrop end
! Yi-Chi add to include triangular PDF   
   select case (macrop_scheme)
   case('updf')
!cjshiu original park's approach
    call uniform_pdf_register()
!cjshiu end orig
   case('tpdf')
    call trig_pdf_register()
   end select
...
! ---------- Yi-Chi end add -------- !
  ...
  ! Find out whether shfrc from convect_shallow will be used in cldfrc
    ...
    ! +++ Yi-Chi : add output for delta and cloud by PDF
    call addfld('DELTALPDF  ', 'K       ', pver,'A', 'width of liquid PDF', phys_decomp )
    call addfld('DELTAIPDF  ', 'K       ', pver,'A', 'width of ice PDF', phys_decomp )
    call addfld('CLDIFUPDF  ', 'K       ', pver,'A', 'ice cloud calculated by PDF', phys_decomp )
    call addfld('CLDLFUPDF  ', 'K       ', pver,'A', 'liquid cloud calculated by PDF', phys_decomp )
    ! --- Yi-Chi
...
!cjshiu add updf macrophysical scheme
  use cldwat2m_macro_updf,   only: mmacro_pcond_updf   
!cjshiu end add
!Yi-Chi add updf macrophysical scheme
  use cldwat2m_macro_tpdf,   only: mmacro_pcond_tpdf
!Yi-Chi end add
...
!cjshiu orig           (4._r8*3.14_r8*10.e-6_r8**3*997._r8)     ! Shallow Convection 
!cjshiu TaiESM.mam3           (4._r8*3.14_r8*12.5e-6_r8**3*997._r8)     ! Shallow Convection 
           (4._r8*3.14_r8*14.0e-6_r8**3*997._r8)     ! Shallow Convection 
    ! dum2                      = dlf(i,k) * dum1
      ptend_loc%q(i,k,ixnumice) = 3._r8 * ( max(0._r8, ( dlf(i,k) - dlf2(i,k) )) *  dum1 ) / &
!cjshiu orig           (4._r8*3.14_r8*25.e-6_r8**3*500._r8) + & ! Deep    Convection
           (4._r8*3.14_r8*15.e-6_r8**3*500._r8) + & ! Deep    Convection
           3._r8 * (                         dlf2(i,k)    *  dum1 ) / &
...
!cjshiu add select case for using U_pdf cloud fraction macrophysical scheme
   select case (macrop_scheme)
   case('park')
!cjshiu original park's approach
...
!cjshiu end orig
   case('updf')
!cjshiu add pbuf
   call mmacro_pcond_updf( lchnk, ncol, dtime, state_loc%pmid, state_loc%pdel,        &
                      t_inout, qv_inout, ql_inout, qi_inout, nl_inout, ni_inout, &                  
                      ttend, qtend, lmitend, itend, nltend, nitend,              &
                      CC_T, CC_qv, CC_ql, CC_qi, CC_nl, CC_ni, CC_qlst,          & 
                      dlf_T, dlf_qv, dlf_ql, dlf_qi, dlf_nl, dlf_ni,             &
                      ...
                      tlat, qvlat, qcten, qiten, ncten, niten,                   &
                      cmeliq, qvadj, qladj, qiadj, qllim, qilim,                 &
                      cld, alst, aist, qlst, qist, do_cldice, pbuf ) 
!cjshiu                      cld, alst, aist, qlst, qist, do_cldice ) 
!cjshiu end add
   case('tpdf')
! Yi-Chi add trig_pdf
   call mmacro_pcond_tpdf( lchnk, ncol, dtime, state_loc%pmid, state_loc%pdel,        &
                      t_inout, qv_inout, ql_inout, qi_inout, nl_inout, ni_inout, &
                      ttend, qtend, lmitend, itend, nltend, nitend,              &
                      CC_T, CC_qv, CC_ql, CC_qi, CC_nl, CC_ni, CC_qlst,          &
                      dlf_T, dlf_qv, dlf_ql, dlf_qi, dlf_nl, dlf_ni,             &
...
! Yi-Chi end add
   case default
        print*,'cjshiu unknown macrophysical options = ',macrop_scheme
   end select
!cjshiu end add select case
