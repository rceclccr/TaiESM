...
! add sgnum by ICTsai
   real(r8) :: sgnum      ! geometric dry mean sigma of the number distribution for aerosol mode
   real(r8) :: sigmaglo    ! lower limit of sgnum
   real(r8) :: sigmaghi    ! upper limit of sgnum
!ICTsai
...
subroutine physprop_get(id, sourcefile, opticstype, &
   ...
   dgnum, dgnumlo, dgnumhi, sgnum, sigmaglo, sigmaghi,&     !add sgnum by ICTsai
...
! add sgnum by ICTsai
   if (present(sgnum))            sgnum            =  physprop(id)%sgnum
   if (present(sigmaglo))         sigmaglo         =  physprop(id)%sigmaglo
   if (present(sigmaghi))         sigmaghi         =  physprop(id)%sigmaghi
!
...
!  Read optics data for modal aerosols
   ...
   ierr = pio_inq_varid(ncid, 'sgnum', vid)
   ierr = pio_get_var(ncid, vid, props%sgnum)

   ierr = pio_inq_varid(ncid, 'sigmaglo', vid)
   ierr = pio_get_var(ncid, vid, props%sigmaglo)

   ierr = pio_inq_varid(ncid, 'sigmaghi', vid)
   ierr = pio_get_var(ncid, vid, props%sigmaghi)
