  ...
  module cldwat2m_macro_tpdf

!cjshiu modified to use full uniform PDF to calculate C and large-scale condensation rate
!cjshiu all the cloud fraction parameterization is for ql and qi but how about the nl and ni
!cjshiu use certain of proportionality to change nl and ni from changes of ql and qi
!cjshiu in the Park's scheme: evaporation of rain is not considered (could this be due to diagnostic rain?)
!cjshiu need to have a parameterization for Sundqvist's 

! Yi-Chi modified cldwat2m_macro_tpdf for triangular distribution
!   based on CJ's code for uniform distribution.
  ...
  !--------------------------------------------------- !
   ...
   ! +++ Yi-Chi +++ !
   ! add subroutines for triang PDF
   use cldfrc_triang_macro
   ! --- Yi-Chi --- !
   ...
   public :: ini_macro, mmacro_pcond_tpdf, aist_vector, aist_single, astG_RHU_single, CAMstfrac
!   public :: ini_macro, mmacro_pcond_tpdf, aist_vector, aist_single, astG_RHU_single, astG_PDF_single, CAMstfrac
!cjshiu add uniform pdf parameters in pbuf register
!   public :: uniform_pdf_register
! +++ Yi-Chi +++ !
!ycwang add triang pdf parameter
   public :: trig_pdf_register
   public :: astG_triangPDF_single, astG_triangPDF
! --- Yi-Chi --- !
...
!cjshiu   real(r8), parameter          :: qist_min     = 1.e-6_r8      ! Minimum in-stratus ice IWC constraint [ kg/kg ]
...
!cjshiu orig   integer,  parameter          :: niter        = 2             ! For iterative computation of QQ with 'ramda' below.
   integer,  parameter          :: niter        = 1             ! For iterative computation of QQ with 'ramda' below.
...
!cjshiu add
   integer :: deltal_idx, deltai_idx, cldlfracuni_idx, cldifracuni_idx, cldtfracuni_idx
   integer :: qscwat_idx, qscice_idx
!cjshiu need to check whether initial value -1 needed or not
!cjshiu end add
...
!cjshiu add 
!cjshiu   use time_manager,   only: is_first_step
...
!cjshiu add subroutine for registering parameters of uniform PDF 
subroutine trig_pdf_register()
!cjshiu
     !------------------------------------------------ !
     ! Register physics buffer fields and constituents !
     !------------------------------------------------ ! 
     use physics_buffer,      only : pbuf_add_field, dtype_r8, dtype_i4,pbuf_times
     
     ! Add fields to physics buffer
!cjshiu add delta and C in physics buffer (need to clraify the difference between 'global' and 'physpkg' in pbuf_add)
! use 'global' first 
! can do the comparison between CLDO and CLDFUPDF
    call pbuf_add_field('DELTALPDF',    'global', dtype_r8,(/pcols,pver,pbuf_times/),    deltal_idx)        ! delta of uniform PDF assumption for cloud liquid water
    call pbuf_add_field('DELTAIPDF',    'global', dtype_r8,(/pcols,pver,pbuf_times/),    deltai_idx)        ! delta of uniform PDF assumption for cloud ice water
    call pbuf_add_field('CLDLFUPDF',    'global', dtype_r8,(/pcols,pver,pbuf_times/),    cldlfracuni_idx)  ! cloud fraction of uniform PDF assumption
    call pbuf_add_field('CLDIFUPDF',    'global', dtype_r8,(/pcols,pver,pbuf_times/),    cldifracuni_idx)  ! cloud fraction of uniform PDF assumption
    call pbuf_add_field('CLDTFUPDF',    'global', dtype_r8,(/pcols,pver,pbuf_times/),    cldtfracuni_idx)  ! cloud fraction of uniform PDF assumption
    call pbuf_add_field('QSCWATUPDF',   'global', dtype_r8,(/pcols,pver,pbuf_times/),    qscwat_idx)  ! saturation specific humidity of water
    call pbuf_add_field('QSCICEUPDF',   'global', dtype_r8,(/pcols,pver,pbuf_times/),    qscice_idx)  ! saturation specific humidity of ice
!cjshiu add end

end subroutine trig_pdf_register
!cjshiu end add sub
...
!cjshiu add pbuf into the following subroutine

   subroutine mmacro_pcond_tpdf( lchnk      , ncol       , dt         , p            , dp         ,              &
                            ...
                            cld        , al_st_star , ai_st_star , ql_st_star   , qi_st_star , do_cldice, pbuf  )
!cjshiu orig                            cld        , al_st_star , ai_st_star , ql_st_star   , qi_st_star , do_cldice  )
...
!cjshiu 
!cjshiu cesm1.0.3 version   use phys_buffer,     only: pbuf_get_fld_idx, pbuf_size_max, pbuf_fld, pbuf_old_tim_idx
   use physics_buffer,  only : physics_buffer_desc, pbuf_get_field, pbuf_set_field, pbuf_get_index, pbuf_old_tim_idx
!cjshiu 

   implicit none

!cjshiu add pbuf
!cjshiu cesm1.0.3   type(pbuf_fld),      intent(inout), dimension(pbuf_size_max) :: pbuf
...
!cjhiu add local variables for uniform PDF cloud fraction
   real(r8)  p_inpdf(pcols,pver)
   real(r8)  T_inpdf(pcols,pver)
   real(r8)  qv_inpdf(pcols,pver)
   real(r8)  ql_inpdf(pcols,pver)
   real(r8)  qi_inpdf(pcols,pver)
!cjshiu add
   integer :: itim                      ! time  index in pbuf
!cjshiu cesm1.0.3   integer :: deltal_idx, deltai_idx, cldlfracuni_idx, cldifracuni_idx, cldtfracuni_idx
!cjshiu cesm1.0.3   integer :: qscwat_idx

   real(r8), pointer  :: deltalpdf(:,:)         ! TBA
   real(r8), pointer  :: deltaipdf(:,:)         ! TBA
   real(r8), pointer  :: cldlfupdf(:,:)         ! TBA
   real(r8), pointer  :: cldifupdf(:,:)         ! TBA
   real(r8), pointer  :: cldtfupdf(:,:)         ! TBA

   real(r8), pointer  :: qscwatupdf(:,:)        ! TBA
   real(r8), pointer  :: qsciceupdf(:,:)        ! TBA
!cjshiu add end

  real(r8) deltalpdf_out(pcols,pver)         ! liquid cloud delta value
  real(r8) cldlfupdf_out(pcols,pver)         ! liquid cloud fraction
  real(r8) deltaipdf_out(pcols,pver)         ! ice cloud delta value
  real(r8) cldifupdf_out(pcols,pver)         ! ice cloud fraction
  real(r8) deltatpdf_out(pcols,pver)         ! total cloud water (ice+liquid) delta value
  real(r8) cldtfupdf_out(pcols,pver)         ! total cloud fraction computed by (ice+liquid) pdf
  real(r8) cldmfupdf_out(pcols,pver)         ! max of icd & liquid cloud fraction
  real(r8) qscwatupdf_out(pcols,pver)
  real(r8) qsciceupdf_out(pcols,pver)
!cjshiu end add local variables

   cone            = 0.999_r8
   qsmall          = 1.e-18_r8
   zeros(:ncol,:)  = 0._r8

!cjshiu get values from physical buffer already calculated after microphysical scheme
     deltal_idx      = pbuf_get_index('DELTALPDF')
     deltai_idx      = pbuf_get_index('DELTAIPDF')
     cldlfracuni_idx     = pbuf_get_index('CLDLFUPDF')
     cldifracuni_idx     = pbuf_get_index('CLDIFUPDF')
     cldtfracuni_idx     = pbuf_get_index('CLDTFUPDF')
     qscwat_idx     = pbuf_get_index('QSCWATUPDF')
     qscice_idx     = pbuf_get_index('QSCICEUPDF')
!cjshiu add end

!cjshiu add delta and C for uniform PDF cloud fraction
  itim = pbuf_old_tim_idx()
   call pbuf_get_field(pbuf, deltal_idx, deltalpdf, start=(/1,1,itim/), kount=(/pcols,pver,1/))
   call pbuf_get_field(pbuf, deltai_idx, deltaipdf, start=(/1,1,itim/), kount=(/pcols,pver,1/))
   call pbuf_get_field(pbuf, cldlfracuni_idx, cldlfupdf, start=(/1,1,itim/), kount=(/pcols,pver,1/))
   call pbuf_get_field(pbuf, cldifracuni_idx, cldifupdf, start=(/1,1,itim/), kount=(/pcols,pver,1/))
   call pbuf_get_field(pbuf, cldtfracuni_idx, cldtfupdf, start=(/1,1,itim/), kount=(/pcols,pver,1/))
   call pbuf_get_field(pbuf, qscwat_idx, qscwatupdf, start=(/1,1,itim/), kount=(/pcols,pver,1/))
   call pbuf_get_field(pbuf, qscice_idx, qsciceupdf, start=(/1,1,itim/), kount=(/pcols,pver,1/))
!cjshiu cesm1.0.3  deltalpdf => pbuf(deltal_idx)%fld_ptr(1,1:pcols,1:pver,lchnk,itim)
!cjshiu cesm1.0.3  deltaipdf => pbuf(deltai_idx)%fld_ptr(1,1:pcols,1:pver,lchnk,itim)
!cjshiu cesm1.0.3  cldlfupdf => pbuf(cldlfracuni_idx)%fld_ptr(1,1:pcols,1:pver,lchnk,itim)
!cjshiu cesm1.0.3  cldifupdf => pbuf(cldifracuni_idx)%fld_ptr(1,1:pcols,1:pver,lchnk,itim)
!cjshiu cesm1.0.3  cldtfupdf => pbuf(cldtfracuni_idx)%fld_ptr(1,1:pcols,1:pver,lchnk,itim)
!cjshiu cesm1.0.3  qscwatupdf => pbuf(qscwat_idx)%fld_ptr(1,1:pcols,1:pver,lchnk,itim)

  deltalpdf_out(1:ncol,1:pver) = deltalpdf(1:ncol,1:pver)
  deltaipdf_out(1:ncol,1:pver) = deltaipdf(1:ncol,1:pver)
  cldlfupdf_out(1:ncol,1:pver) = cldlfupdf(1:ncol,1:pver)
  cldifupdf_out(1:ncol,1:pver) = cldifupdf(1:ncol,1:pver)
  cldtfupdf_out(1:ncol,1:pver) = cldtfupdf(1:ncol,1:pver)
  qscwatupdf_out(1:ncol,1:pver) = qscwatupdf(1:ncol,1:pver)
  qsciceupdf_out(1:ncol,1:pver) = qsciceupdf(1:ncol,1:pver)

  if( get_nstep() .le. 1 ) then           !cjshiu add this to avoid 1st timestep while nothing is stored in pbuf for the delta and C in after calling microphysics
!cjshiu need to check the possible impacts from the following assumptions
!cjshiu    if (is_first_step()) then
    deltalpdf_out(1:ncol,1:pver) = 1.e-5_r8
    deltaipdf_out(1:ncol,1:pver) = 1.e-5_r8
    cldlfupdf_out(1:ncol,1:pver) = 1.e-5_r8
    cldifupdf_out(1:ncol,1:pver) = 1.e-5_r8
    cldtfupdf_out(1:ncol,1:pver) = 1.e-5_r8
    qscwatupdf_out(1:ncol,1:pver) = 1._r8     !cjshiu needs to check this impact
    qsciceupdf_out(1:ncol,1:pver) = 1._r8     !cjshiu needs to check this impact
   end if
!cjshiu add end 
...
!cjshiu add initialization of local variables for uniform PDF cloud fraction
     p_inpdf(1:ncol,1:pver)  = 0._r8
     T_inpdf(1:ncol,1:pver)  = 0._r8
     qv_inpdf(1:ncol,1:pver)  = 0._r8
     ql_inpdf(1:ncol,1:pver)  = 0._r8
     qi_inpdf(1:ncol,1:pver)  = 0._r8
!cjshiu end add
...
!cjshiu add uniform pdf for al_st and ai_st and update the values storaged in physical buffer calculated after microphysical processes to evaluate the impacts and difference
!cjshiu ignore uniform pdf parameter calculations in microp_driver in CESM1.2.2 version

!cjshiu add
    p_inpdf(:ncol,:) = p(:ncol,:)
    T_inpdf(:ncol,:) = T1(:ncol,:)
    qv_inpdf(:ncol,:) = qv1(:ncol,:)
    ql_inpdf(:ncol,:) = ql1(:ncol,:)
    qi_inpdf(:ncol,:) = qi1(:ncol,:)
!cjshiu    print*,' cjshiu before 1st call of uniform pdf p_inpdf=  ',p_inpdf(:ncol,:)
!cjshiu    print*,' cjshiu before 1st call of uniform pdf T_inpdf=  ',T_inpdf(:ncol,:)
!cjshiu    print*,' cjshiu before 1st call of uniform pdf qv_inpdf=  ',qv_inpdf(:ncol,:)
!cjshiu    print*,' cjshiu before 1st call of uniform pdf ql_inpdf=  ',ql_inpdf(:ncol,:)
!cjshiu    print*,' cjshiu before 1st call of uniform pdf qi_inpdf=  ',qi_inpdf(:ncol,:)
!cjshiu    print*,' before p1 p0_in= ',p0_in(:ncol,:)
!cjshiu    print*,'before call macro_uniformpdf_2d ql1= ',ql1
!cjshiu    print*,'before call macro_uniformpdf_2d qi1= ',qi1
!cjshiu add end

! +++ Yi-Chi +++ !
call macro_triangpdf_2d(ncol,  p_inpdf(:,:), T_inpdf(:,:), qv_inpdf(:,:), ql_inpdf(:,:), qi_inpdf(:,:), &
                         deltalpdf_out(:,:), cldlfupdf_out(:,:), deltaipdf_out(:,:), cldifupdf_out(:,:), &
                         deltatpdf_out(:,:), cldtfupdf_out(:,:), cldmfupdf_out(:,:), qscwatupdf_out(:,:), qsciceupdf_out(:,:))
!cjshiu debugging check for calling 2D macro_uniformpdf    
!call macro_uniformpdf_2d(ncol,  p_inpdf(:,:), T_inpdf(:,:), qv_inpdf(:,:), ql_inpdf(:,:), qi_inpdf(:,:), &  
!                         deltalpdf_out(:,:), cldlfupdf_out(:,:), deltaipdf_out(:,:), cldifupdf_out(:,:), &
!                         deltatpdf_out(:,:), cldtfupdf_out(:,:), cldmfupdf_out(:,:), qscwatupdf_out(:,:), qsciceupdf_out(:,:))
! --- Yi-Chi --- !
!cjshiu    print*,'debugging check 2D ok or not deltal, cldl, deltai, cldi, deltat, cldt, cldm= ',deltalpdf,cldlfupdf,deltaipdf,cldifupdf,deltatpdf,cldtfupdf,cldmfupdf 
!cjshiu debugging end
!cjshiu update physical buffer values 
    do k=1,pver
     do i=1,ncol
!cjshiu    deltatpdf_out(:ncol,:)   
    cldtfupdf(i,k) = cldtfupdf_out(i,k)
    deltaipdf(i,k) = deltaipdf_out(i,k)
    cldifupdf(i,k) = cldifupdf_out(i,k)
    deltalpdf(i,k) = deltalpdf_out(i,k)
    cldlfupdf(i,k) = cldlfupdf_out(i,k)
    qscwatupdf(i,k) = qscwatupdf_out(i,k)
    qsciceupdf(i,k) = qsciceupdf_out(i,k)
!cjshiu    cldmfupdf_out(:ncol,:) 
     enddo
    enddo

! +++ Yi-Chi +++ : turn on the output for debugging !
!cjshiu not work in cesm1.2.2  (shr_sys_abort) ERROR: GET_MASTERLIST_INDX: attemping to output field DELTALPDF not on master list
!cjshiu   
    call outfld( 'DELTALPDF',      deltalpdf,    pcols, lchnk )
!cjshiu   
    call outfld( 'DELTAIPDF',      deltaipdf,    pcols, lchnk )
!cjshiu   
    call outfld( 'CLDLFUPDF',      cldlfupdf,    pcols, lchnk )
!cjshiu   
    call outfld( 'CLDIFUPDF',      cldifupdf,    pcols, lchnk )
!cjshiu   
!    call outfld( 'CLDTFUPDF',      cldtfupdf,    pcols, lchnk )
!cjshiu end add uniform pdf
! --- Yi-Chi --- !
...
!cjshiu add pbuf
      call instratus_condensate( lchnk, ncol, k,                                   &
                                 ...
                                 al_st_0(:,k), ai_st_0(:,k), ql_st_0(:,k), qi_st_0(:,k), pbuf )
!cjshiu orig                                 al_st_0(:,k), ai_st_0(:,k), ql_st_0(:,k), qi_st_0(:,k) )
!cjshiu add end
...
!cjshiu    print*,' cjshiu after T_05=  ',T_05(:ncol,:)
!cjshiu    print*,' cjshiu after qv_05=  ',qv_05(:ncol,:)
!cjshiu    print*,' cjshiu after ql_05=  ',ql_05(:ncol,:)
!cjshiu    print*,' cjshiu after qi_05=  ',qi_05(:ncol,:)
!cjshiu    print*,' cjshiu after nl_05=  ',nl_05(:ncol,:)
!cjshiu    print*,' cjshiu after ni_05=  ',ni_05(:ncol,:)
...
!cjshiu print out some variables for checking purpose
!cjshiu    print *,'cjshiu before main interative ql=',ql
!cjshiu    print *,'cjshiu before main interative qi=',qi
!cjshiu    print *,'cjshiu before main interative al_st=',al_st
!cjshiu    print *,'cjshiu before main interative ai_st=',ai_st
!cjshiu    print *,'cjshiu before main interative ql_st=',ql_st
!cjshiu    print *,'cjshiu before main interative qi_st=',qi_st

!cjshiu what's the purpose for doing interative calculations?
!cjshiu Dr. Pan suggest to simply use the ql and qi after previous timestep for getting delta and C
!cjshiu later should check how the results will change if using ql and qi after the iteration of Park's way
...
! ycwang replaces the uniform with triangle
      call astG_triangPDF(U_nc(:,k),p(:,k),qv(:,k),landfrac(:),snowh(:),al_st_nc(:,k),G_nc(:,k),ncol,k,lchnk,pbuf)
!cjshiu use uniform pdf for cloud liquid
!          call astG_PDF(U_nc(:,k),p(:,k),qv(:,k),landfrac(:),snowh(:),al_st_nc(:,k),G_nc(:,k),ncol,k,lchnk,pbuf)
!cjshiu orig          call astG_PDF(U_nc(:,k),p(:,k),qv(:,k),landfrac(:),snowh(:),al_st_nc(:,k),G_nc(:,k),ncol)
      endif
!cjshiu use uniform pdf for cloud ice
      call aist_vector(qv(:,k),T(:,k),p(:,k),qi(:,k),landfrac(:),snowh(:),ai_st_nc(:,k),ncol,k,lchnk,pbuf)
!cjshiu orig      call aist_vector(qv(:,k),T(:,k),p(:,k),qi(:,k),landfrac(:),snowh(:),ai_st_nc(:,k),ncol)
...
!cjshiu    print*,' cjshiu after T_dprime=  ',T_dprime(:ncol,:)
!cjshiu    print*,' cjshiu after qv_dprime=  ',qv_dprime(:ncol,:)
!cjshiu    print*,' cjshiu after ql_dprime=  ',ql_dprime(:ncol,:)
!cjshiu    print*,' cjshiu after qi_dprime=  ',qi_dprime(:ncol,:)
!cjshiu    print*,' cjshiu after nl_dprime=  ',nl_dprime(:ncol,:)
!cjshiu    print*,' cjshiu after ni_dprime=  ',ni_dprime(:ncol,:)
...
!cjshiu add uniform pdf for al_st and ai_st and update the values storaged in physical buffer calculated after microphysical processes to evaluate the impacts and difference
!cjshiu    print*,' cjshiu after add cumulus T_dprime=  ',T_dprime(:ncol,:)
!cjshiu    print*,' cjshiu after add cumulus qv_dprime=  ',qv_dprime(:ncol,:)
!cjshiu    print*,' cjshiu after add cumulus ql_dprime=  ',ql_dprime(:ncol,:)
!cjshiu    print*,' cjshiu after add cumulus qi_dprime=  ',qi_dprime(:ncol,:)
!cjshiu    print*,' cjshiu after add cumulus nl_dprime=  ',nl_dprime(:ncol,:)
!cjshiu    print*,' cjshiu after add cumulus ni_dprime=  ',ni_dprime(:ncol,:)
! remove this to check this cause in negative values of qv

!cjshiu need to check the location for updating uniform PDF cloud fraction

!cjshiu add
!cjshiu    p_inpdf(:ncol,:) = p(:ncol,:)
!cjshiu    T_inpdf(:ncol,:) = T_dprime(:ncol,:)
!cjshiu    qv_inpdf(:ncol,:) = qv_dprime(:ncol,:)
!cjshiu    ql_inpdf(:ncol,:) = ql_dprime(:ncol,:)
!cjshiu    qi_inpdf(:ncol,:) = qi_dprime(:ncol,:)
!cjshiu    print*,' cjshiu before second call of uniform pdf p_inpdf=  ',p_inpdf(:ncol,:)
!cjshiu    print*,' cjshiu before second call of uniform pdf T_inpdf=  ',T_inpdf(:ncol,:)
!cjshiu    print*,' cjshiu before second call of uniform pdf qv_inpdf=  ',qv_inpdf(:ncol,:)
!cjshiu    print*,' cjshiu before second call of uniform pdf ql_inpdf=  ',ql_inpdf(:ncol,:)
!cjshiu    print*,' cjshiu before second call of uniform pdf qi_inpdf=  ',qi_inpdf(:ncol,:)
!cjshiu add end

!cjshiu debugging check for calling 2D macro_uniformpdf
!cjshiu    call macro_uniformpdf_2d(ncol,  p_inpdf(:,:), T_inpdf(:,:), qv_inpdf(:,:), ql_inpdf(:,:), qi_inpdf(:,:), &
!cjshiu                    deltalpdf_out(:,:), cldlfupdf_out(:,:), deltaipdf_out(:,:), cldifupdf_out(:,:), &
!cjshiu                    deltatpdf_out(:,:), cldtfupdf_out(:,:), cldmfupdf_out(:,:), qscwatupdf_out(:,:), qsciceupdf_out(:,:)  )
!cjshiu    print*,'debugging check 2D ok or not deltal, cldl, deltai, cldi, deltat, cldt, cldm= ',deltalpdf,cldlfupdf,deltaipdf,cldifupdf,deltatpdf,cldtfupdf,cldmfupdf 
!cjshiu debugging end
!cjshiu update physical buffer values 
!cjshiu    do k=1,pver
!cjshiu     do i=1,ncol
!cjshiu no use    deltatpdf_out(:ncol,:)   
!cjshiu    cldtfupdf(i,k) = cldtfupdf_out(i,k)
!cjshiu    deltaipdf(i,k) = deltaipdf_out(i,k)
!cjshiu    cldifupdf(i,k) = cldifupdf_out(i,k)
!cjshiu    deltalpdf(i,k) = deltalpdf_out(i,k)
!cjshiu    cldlfupdf(i,k) = cldlfupdf_out(i,k)
!cjshiu    qscwatupdf(i,k) = qscwatupdf_out(i,k)
!cjshiu    qsciceupdf(i,k) = qsciceupdf_out(i,k)
!cjshiu    cldmfupdf_out(:ncol,:) 
!cjshiu     enddo
!cjshiu    enddo

!cjshiu not work for cesm1.2.2 (shr_sys_abort) ERROR: GET_MASTERLIST_INDX: attemping to output field DELTALPDF not on master list
!cjshiu   call outfld( 'DELTALPDF',      deltalpdf,    pcols, lchnk )
!cjshiu   call outfld( 'DELTAIPDF',      deltaipdf,    pcols, lchnk )
!cjshiu   call outfld( 'CLDLFUPDF',      cldlfupdf,    pcols, lchnk )
!cjshiu   call outfld( 'CLDIFUPDF',      cldifupdf,    pcols, lchnk )
!cjshiu   call outfld( 'CLDTFUPDF',      cldtfupdf,    pcols, lchnk )
...
!cjshiu add pbuf
      call instratus_condensate( lchnk          , ncol           , k              , p(:,k)        , &
                                 ...
                                 al_st_star(:,k), ai_st_star(:,k), ql_st_star(:,k), qi_st_star(:,k), pbuf )
!cjshiu orig                                 al_st_star(:,k), ai_st_star(:,k), ql_st_star(:,k), qi_st_star(:,k) )
...
!cjshiu   print*,'cjshiu end of mmacro_pcond ql0= ',ql0
!cjshiu   print*,'cjshiu end of mmacro_pcond qi0= ',qi0
!cjshiu   print*,'cjshiu end of mmacro_pcond al_st= ',al_st
!cjshiu   print*,'cjshiu end of mmacro_pcond ai_st= ',ai_st
!cjshiu   print*,'cjshiu end of mmacro_pcond nl0= ',nl0
!cjshiu   print*,'cjshiu end of mmacro_pcond ni0= ',ni0
!cjshiu   print*,'cjshiu end of mmacro_pcond qv0= ',qv0
!cjshiu   print*,'cjshiu end of mmacro_pcond T0= ',T0

   return
   end subroutine mmacro_pcond_tpdf
   ...
   subroutine instratus_condensate( lchnk, ncol, k,                      &  
                                    ...
                                    al_st_out, ai_st_out, ql_st_out, qi_st_out, pbuf )
!cjshiu orig                                    al_st_out, ai_st_out, ql_st_out, qi_st_out )
...
!cjshiu add end
   ...
   ! +++ Yi-Chi +++ !
   ! ycwang replace uniform with triangular
       call astG_triangPDF(U0_in(:),p_in(:),qv0_in(:),landfrac(:),snowh(:),al0_st_nc_in(:),G0_nc_in(:),ncol,k,lchnk,pbuf)
   !    call astG_PDF(U0_in(:),p_in(:),qv0_in(:),landfrac(:),snowh(:),al0_st_nc_in(:),G0_nc_in(:),ncol,k,lchnk,pbuf)
   ! --- Yi-Chi --- !
!cjshiu orig       call astG_PDF(U0_in(:),p_in(:),qv0_in(:),landfrac(:),snowh(:),al0_st_nc_in(:),G0_nc_in(:),ncol)
   endif
   call aist_vector(qv0_in(:),T0_in(:),p_in(:),qi0_in(:),landfrac(:),snowh(:),ai0_st_nc_in(:),ncol,k,lchnk,pbuf)
!cjshiu orig   call aist_vector(qv0_in(:),T0_in(:),p_in(:),qi0_in(:),landfrac(:),snowh(:),ai0_st_nc_in(:),ncol)
...
! +++ Yi-Chi +++ !
              call astG_triangPDF_single(U0_nc,p,qv0,landfrac(i),snowh(i),al0_st_nc,G0_nc,i,k,lchnk,pbuf)
!              call astG_PDF_single(U0_nc,p,qv0,landfrac(i),snowh(i),al0_st_nc,G0_nc,i,k,lchnk,pbuf)
!cjshiu orig              call astG_PDF_single(U0_nc,p,qv0,landfrac(i),snowh(i),al0_st_nc,G0_nc)
! --- Yi-Chi --- !
          endif
          call aist_single(qv0,T0,p,qi0,landfrac(i),snowh(i),ai0_st_nc,i,k,lchnk,pbuf)
!cjshiu orig          call aist_single(qv0,T0,p,qi0,landfrac(i),snowh(i),ai0_st_nc)
...
! +++ Yi-Chi +++ !
! ychwang adds triangular
             call astG_triangPDF_single(U_nc,p,qv,landfrac(i),snowh(i),al_st_nc,G_nc,i,k,lchnk,pbuf)
!                 call astG_PDF_single(U_nc,p,qv,landfrac(i),snowh(i),al_st_nc,G_nc,i,k,lchnk,pbuf)
!cjshiu orig                 call astG_PDF_single(U_nc,p,qv,landfrac(i),snowh(i),al_st_nc,G_nc)
! --- Yi-Chi --- !
             endif
...
10 continue   
   ...
   ! -------------------------------------------------- !
     ...
     if( idxmod .eq. 1 ) then
         call aist_single(qv,T,p,qi,landfrac(i),snowh(i),ai_st_nc,i,k,lchnk,pbuf)
!cjshiu orig         call aist_single(qv,T,p,qi,landfrac(i),snowh(i),ai_st_nc)
...
! +++ Yi-Chi +++ !
         call astG_triangPDF_single(U_nc,p,qv,landfrac(i),snowh(i),al_st_nc,G_nc,i,k,lchnk,pbuf)
!             call astG_PDF_single(U_nc,p,qv,landfrac(i),snowh(i),al_st_nc,G_nc,i,k,lchnk,pbuf)
!cjshiu             call astG_PDF_single(U_nc,p,qv,landfrac(i),snowh(i),al_st_nc,G_nc)
! --- Yi-Chi --- !
         endif
         al_st = (1._r8-a_dc-a_sc)*al_st_nc
     else
     ...
     if( al_st .eq. 0._r8 ) then
         ...
         ql_st = ql/al_st
!cjshiu orig         ql_st = min(qlst_max,max(qlst_min,ql_st)) ! PJR
!cjshiu remove this can significantly fix the negative qv in the upper 12 levels
!cjshiu now only level 7 to 13 will occasionally show negatvie qv --> one or two layers  
     endif
...
!cjshiu need to figure out the occurrence of negative qv cuased by larger value of ql produced from adjustment of Park's
!cjshiu physical meaningful way should be used for fix this bug
!cjshiu the following is temporarily tried (i.e. ignore ql change)
   if (qv .LT. 0._r8) then
    T  = T0 - ((latvap+latice)/cpair)*(qi0-qi)
    qv = qv0 + qi0 - qi
!cjshiu    print*,'cjshiu in instratus_condensate T, T0= ',T,T0
!cjshiu    print*,'cjshiu in instratus_condensate qv, qv0, ql0, ql, qi0, qi=',qv,qv0,ql0,ql,qi0,qi
   end if
...
!cjshiu add end
   ...
   call funcd_instratus( x1, p, T0, qv0, ql0, qi0, fice0, muQ0, qc_nc0, &
                         ...
                         fl, df, qc_nc, fice, al_st, icol, k, lchnk, pbuf )
!cjshiu orig                         fl, df, qc_nc, fice, al_st )
   call funcd_instratus( x2, p, T0, qv0, ql0, qi0, fice0, muQ0, qc_nc0, &
                         a_dc, ql_dc, qi_dc, a_sc, ql_sc, qi_sc, ai_st, &
                         qcst_crit, landfrac, snowh,                    &
                         fh, df, qc_nc, fice, al_st, icol, k, lchnk, pbuf )
!cjshiu orig                         fh, df, qc_nc, fice, al_st )
...
!cjshiu orig                             fl, df, qc_nc, fice, al_st )
   ...
   call funcd_instratus( rtsafe, p, T0, qv0, ql0, qi0, fice0, muQ0, qc_nc0, &
                         a_dc, ql_dc, qi_dc, a_sc, ql_sc, qi_sc, ai_st,     &
                         qcst_crit, landfrac, snowh,                        &
                         f, df, qc_nc, fice, al_st, icol, k, lchnk, pbuf )
!cjshiu orig                         f, df, qc_nc, fice, al_st )
   do j = 1, 20
...
10 continue
   ...
   subroutine funcd_instratus( T, p, T0, qv0, ql0, qi0, fice0, muQ0, qc_nc0,   &
                               a_dc, ql_dc, qi_dc, a_sc, ql_sc, qi_sc, ai_st,  &
                               qcst_crit, landfrac, snowh,                     &
                               f, fg, qc_nc, fice, al_st, icol, k, lchnk, pbuf ) 
!cjshiu orig                               f, fg, qc_nc, fice, al_st ) 
...
!cjshiu add for using pbuf
   use physics_buffer,  only : physics_buffer_desc, pbuf_get_field, pbuf_set_field,pbuf_times
!cjshiu add end
...
! +++ Yi-Chi +++ !
       call astG_triangPDF_single(U_nc,p,qv,landfrac,snowh,al_st_nc,G_nc,icol,k,lchnk,pbuf)
!cjshiu uniform       call astG_PDF_single(U_nc,p,qv,landfrac,snowh,al_st_nc,G_nc,icol,k,lchnk,pbuf)
!cjshiu orig       call astG_PDF_single(U_nc,p,qv,landfrac,snowh,al_st_nc,G_nc)
! --- Yi-Chi --- !
   endif
!cjshiu   print*,'cjshiu in funcd_instratus al_st_nc, G_nc= ,',al_st_nc,G_nc
   al_st   =  (1._r8-a_dc-a_sc)*al_st_nc 
...
11 continue
   ...
   ! ----------------- !
   ! Yi-Chi removes subroutine astG_PDF_single
   ! ----------------- !
!cjshiu note the orhmin variable (optional parameter) --> need to check this impact
!   subroutine astG_PDF_single( U, p, qv, landfrac, snowh, a, Ga, icol, k, lchnk, pbuf, orhmin )
!cjshiu   subroutine astG_PDF_single( U, p, qv, landfrac, snowh, a, Ga, orhmin )
   ...
   ! ----------------- !
    ...
    ! ----------------- !
   ! Yi-Chi removes subroutine astG_PDF
   ! ----------------- !
!   subroutine astG_PDF( U_in, p_in, qv_in, landfrac_in, snowh_in, a_out, Ga_out, ncol, k, lchnk,  pbuf )
!cjshiu orig   subroutine astG_PDF( U_in, p_in, qv_in, landfrac_in, snowh_in, a_out, Ga_out, ncol )
   ...
   subroutine aist_single( qv, T, p, qi, landfrac, snowh, aist, icol, k, lchnk, pbuf )
!cjshiu orig   subroutine aist_single( qv, T, p, qi, landfrac, snowh, aist )
...
!cjshiu add column and level index number
   integer :: itim                      ! time  index in pbuf
...
!cjshiu add 
   integer :: deltai_idx, cldifracuni_idx

   real(r8), pointer, dimension(:,:) :: deltaipdf         ! TBA
   real(r8), pointer, dimension(:,:) :: cldifupdf         ! TBA
     
!cjshiu add end
!cjshiu add local variables for uniform PDF to pass values from physical buffer values for calculation
   real(r8), dimension(pcols,pver) :: deltai         ! TBA 
   real(r8), dimension(pcols,pver) :: cldifu         ! TBA
!cjshiu add end
...
!cjshiu add delta and C for uniform PDF cloud fraction
  ...
  deltai(1:icol,1:pver) = deltaipdf(1:icol,1:pver)
  cldifu(1:icol,1:pver) = cldifupdf(1:icol,1:pver)

  if( get_nstep() .le. 1 ) then           !cjshiu add this to avoid 1st timestep while nothing is stored in pbuf for the delta and C in after calling microphysics
!cjshiu  if( is_first_step() ) then           !cjshiu add this to avoid 1st timestep while nothing is stored in pbuf for the delta and C in after calling microphysics
  deltai(1:icol,1:pver) = 1.e-5_r8
  cldifu(1:icol,1:pver) = 1.e-5_r8
  endif
!cjshiu add end
...
!cjshiu use uniform PDF for cloud ice already calculated in microp_driver.F90
     aist = cldifu(icol,k)
     aist = max(0._r8,min(aist,0.999_r8))
!cjshiu end 
!cjshiu    print*,' cjshiu in aist single icol k aist=',icol,k,aist
   ...
   subroutine aist_vector( qv_in, T_in, p_in, qi_in, landfrac_in, snowh_in, aist_out, ncol, k, lchnk,  pbuf )
!cjshiu orig   subroutine aist_vector( qv_in, T_in, p_in, qi_in, landfrac_in, snowh_in, aist_out, ncol )
...
!cjshiu add delta and C for uniform PDF cloud fraction
  ...
  deltai(1:ncol,1:pver) = deltaipdf(1:ncol,1:pver)
  cldifu(1:ncol,1:pver) = cldifupdf(1:ncol,1:pver)

  if( get_nstep() .le. 1 ) then           !cjshiu add this to avoid 1st timestep while nothing is stored in pbuf for the delta and C in after calling microphysics
!cjshiu  if( is_first_step() ) then           !cjshiu add this to avoid 1st timestep while nothing is stored in pbuf for the delta and C in after calling microphysics
  deltai(1:ncol,1:pver) = 1.e-5_r8
  cldifu(1:ncol,1:pver) = 1.e-5_r8
  endif
!cjshiu add end
...
! set rh ice cloud fraction
             rhi= (qv+qi)/qs * (esl/esi)
!cjshiu check
!cjshiu             print*,'cjshiu in aist_vector rhi= ',rhi 
!cjshiu check end
             rhdif= (rhi-rhmini) / (rhmaxi - rhmini)
             aist = min(1.0_r8, max(rhdif,0._r8)**2)
...
!cjshiu use uniform PDF for cloud ice already calculated in microp_driver.F90
     aist_out(i) = cldifu(i,k) 
     aist_out(i) = max(0._r8,min(aist_out(i),0.999_r8))
!cjshiu end
...
24    continue
   ...
   ! ---------------------------------- !
   ! Yi-Chi adds triangular subroutines 
   ! ---------------------------------- !
   subroutine astG_triangPDF_single( U, p, qv, landfrac, snowh, a, Ga, icol, k, lchnk, pbuf, orhmin )
!cjshiu   subroutine astG_PDF_single( U, p, qv, landfrac, snowh, a, Ga, orhmin )
...
!cjshiu add column and level index number
   ...
   integer,  intent(in)    :: lchnk                        ! Chunk number
   integer, intent(in) :: k
   real(r8), parameter :: us = 1.0_r8
!cjshiu add end
! ycwang add small value
   real(r8), parameter :: small    = 10._r8**(-10)
! ycwang add end
   ! Local variables
...
!cjshiu add local variables for uniform PDF to pass values from physical buffer values for calculation
   real(r8), dimension(pcols,pver) :: deltal         ! TBA
   real(r8), dimension(pcols,pver) :: deltai         ! TBA
   real(r8), dimension(pcols,pver) :: cldlfu         ! TBA
   real(r8), dimension(pcols,pver) :: cldifu         ! TBA
   real(r8), dimension(pcols,pver) :: cldtfu         ! TBA

   real(r8), dimension(pcols,pver) :: qscwat         ! TBA
   real(r8), dimension(pcols,pver) :: qscice         ! TBA
!cjshiu add end
...
!cjshiu add delta and C for uniform PDF cloud fraction
  ...
  deltal(1:icol,1:pver) = deltalpdf(1:icol,1:pver)
  deltai(1:icol,1:pver) = deltaipdf(1:icol,1:pver)
  cldlfu(1:icol,1:pver) = cldlfupdf(1:icol,1:pver)
  cldifu(1:icol,1:pver) = cldifupdf(1:icol,1:pver)
  cldtfu(1:icol,1:pver) = cldtfupdf(1:icol,1:pver)
  qscwat(1:icol,1:pver) = qscwatupdf(1:icol,1:pver)
  qscice(1:icol,1:pver) = qsciceupdf(1:icol,1:pver)

  if( get_nstep() .le. 1 ) then           !cjshiu add this to avoid 1st timestep while nothing is stored in pbuf for the delta and C in after calling microphysics
!cjshiu  if( is_first_step() ) then           !cjshiu add this to avoid 1st timestep while nothing is stored in pbuf for the delta and C in after calling microphysics
  deltal(1:icol,1:pver) = 1.e-5_r8
  deltai(1:icol,1:pver) = 1.e-5_r8
  cldlfu(1:icol,1:pver) = 1.e-5_r8
  cldifu(1:icol,1:pver) = 1.e-5_r8
  cldtfu(1:icol,1:pver) = 1.e-5_r8
  qscwat(1:icol,1:pver) = 1._r8
  qscice(1:icol,1:pver) = 1._r8
  endif
...
!cjshiu add end
       ...
       elseif( U .gt. (cldrh-dV) .and. U .le. (cldrh-dV/6._r8) ) then
           ...
           ! +++ Yi-Chi +++ !
           ! ycwang adds a ~0 case !
            if(a.gt.small)then
             Ga = dV/sqrt(2._r8)*(1._r8/sqrt(a)-sqrt(a))
           else
           !if(cldfrc.le.small)then
             Ga = 1.e10_r8
           endif
           !Ga = dV/sqrt(2._r8)*(1._r8/sqrt(a)-sqrt(a))
...
!cjshiu add uniform PDF for a and Ga here via calling variables already derived after microphysics of previous timestep
! assume only process for cloud liquid water only (qscwat of cloud liquid phase, need to change all to Sundviquest's later)
! need to confirm dU/da = 2(1-b)*delta/qs+Us-1
!   a =  cldlfu(icol,k)
!   Ga = 2._r8*(1._r8 - a)*deltal(icol,k)/qscwat(icol,k) + us - 1._r8

!cjshiu   print*,'cjshu in astG_PDF_single icol= k= deltal= qscwat= a= Ga= ',icol,k,deltal(icol,k),qscwat(icol,k),a,Ga

!!cjshiu add the following to avoid a=1 and Ga= 0 condition that will cause floating divided by zero
!cjshiu   if (a .ge. 1._r8) then
   if (a .ge. 0.9999_r8) then
    a = 1._r8
    Ga = 1.e10_r8  !cjshiu following given a large number (need to check this impact)
   endif
!!cjshiu end add

!cjshiu   print*,'a= Ga= in astG_PDF_single',a,Ga
!cjshiu add end

   if (present(orhmin)) orhmin = rhmin

   return
   end subroutine astG_triangPDF_single
   ! ----------------- !
   ! End of subroutine !
   ! ----------------- !


   subroutine astG_triangPDF( U_in, p_in, qv_in, landfrac_in, snowh_in, a_out, Ga_out, ncol, k, lchnk,  pbuf )
!cjshiu orig   subroutine astG_PDF( U_in, p_in, qv_in, landfrac_in, snowh_in, a_out, Ga_out, ncol )
...
!cjshiu add
   ...
   real(r8), pointer, dimension(:,:) :: deltalpdf         ! TBA
   real(r8), pointer, dimension(:,:) :: deltaipdf         ! TBA
   real(r8), pointer, dimension(:,:) :: cldlfupdf         ! TBA
   real(r8), pointer, dimension(:,:) :: cldifupdf         ! TBA
   real(r8), pointer, dimension(:,:) :: cldtfupdf         ! TBA

   real(r8), pointer, dimension(:,:) :: qscwatupdf        ! TBA
   real(r8), pointer, dimension(:,:) :: qsciceupdf        ! TBA
!cjshiu add end
...
!cjshiu add delta and C for uniform PDF cloud fraction
  ...
  deltal(1:ncol,1:pver) = deltalpdf(1:ncol,1:pver)
  deltai(1:ncol,1:pver) = deltaipdf(1:ncol,1:pver)
  cldlfu(1:ncol,1:pver) = cldlfupdf(1:ncol,1:pver)
  cldifu(1:ncol,1:pver) = cldifupdf(1:ncol,1:pver)
  cldtfu(1:ncol,1:pver) = cldtfupdf(1:ncol,1:pver)
  qscwat(1:ncol,1:pver) = qscwatupdf(1:ncol,1:pver)
  qscice(1:ncol,1:pver) = qsciceupdf(1:ncol,1:pver)

!cjshiu   print*,'cjshiu deltal= ',deltal
!cjshiu   print*,'cjshiu cldlfu= ',cldlfu
!cjshiu   print*,'cjshiu qscwat= ',qscwat
!cjshiu   print*,'cjshiu qscice= ',qscice

  if( get_nstep() .le. 1 ) then           !cjshiu add this to avoid 1st timestep while nothing is stored in pbuf for the delta and C in after calling microphysics
!cjshiu  if( is_first_step() ) then           !cjshiu add this to avoid 1st timestep while nothing is stored in pbuf for the delta and C in after calling microphysics
  deltal(1:ncol,1:pver) = 1.e-5_r8
  deltai(1:ncol,1:pver) = 1.e-5_r8
  cldlfu(1:ncol,1:pver) = 1.e-5_r8
  cldifu(1:ncol,1:pver) = 1.e-5_r8
  cldtfu(1:ncol,1:pver) = 1.e-5_r8
  qscwat(1:ncol,1:pver) = 1._r8
  qscice(1:ncol,1:pver) = 1._r8
  endif
!cjshiu add end
...
!cjshiu in astG_PDF
!cjshiu    print*,'cjshiu check qv in astG_PDF qv=',qv
...
! need to confirm dU/da = 2(1-b)*delta/qs+Us-1
!   a_out(i) =  cldlfu(i,k)
!   Ga_out(i) = 2._r8*(1._r8 - a_out(i))*deltal(i,k)/qscwat(i,k) + us - 1._r8
!cjshiu add end
! --- Yi-Chi --- !

!cjshiu add the following to avoid a=1 and Ga= 0 condition that will cause floating divided by zero
!cjshiu   if (a_out(i) .ge. 1._r8) then
   if (a_out(i) .ge. 0.9999_r8) then
    a_out(i) = 1._r8
    Ga_out(i) = 1.e10_r8  !cjshiu following given a large number (need to check this impact)
   endif
!!cjshiu end add
   ...
   return
   end subroutine astG_triangPDF

   ! ----------------- !
   ! End of subroutine !
   ! ----------------- !

end module cldwat2m_macro_tpdf
