...
! ------------------------------
! Yi-Chi (June 2016)
! - apply the changes of ZM scheme from CESM 2016 workshop
!   replace the 5 time calculation of CAPE with 1 time
! ------------------------------
! Yi-Chi (April 2015)
! - modify the subroutines for initialization and namelist reading
!   to accommodate ZMMOD as an option for CAM5.3.
!   The subroutine zmmodconv_readnl is now called in zmmod_convi
!    to allow ZMMOD reading in namelist-set parameters.
! Yi-Chi (Oct 2014)
! - move triggering function to CAM5.3
! - allow cloud bottom above PBL but stay below 600hPa.
! - change the LCL to LFC
!---------------------------------------------------------------------------------
...
! PUBLIC: interfaces
!
  public zmmodconv_readnl            ! read zmconv_nl namelist
  public zmmod_convi                 ! ZM schemea
  ! --- change zm_convr ---
...
! +++ Yi-Chi
!    Remove the zmconv_readnl 
! --- Yi-Chi --- !
subroutine zmmodconv_readnl(nlfile)
...
end subroutine zmmodconv_readnl


subroutine zmmod_convi(limcnv_in, no_deep_pbl_in)
...
!
   ...
   ! +++ Yi-Chi +++ !
   character(len=32) :: nlfilename = 'atm_in' ! Namelist filename
   ! --- Yi-Chi --- !
   ...
   if ( masterproc ) then
      write(iulog,*) 'tuning parameters zmmod_convi: tau',tau
...
end subroutine zmmod_convi



subroutine zmmod_convr(lchnk   ,ncol    , &
...
! +++ Yi-Chi : Sept 2013 : adding vertical velocity
                    lengath ,ql      ,rliq    ,landfrac, omega)
...
!
   ...
   ! +++ Yi-Chi
   real(r8), intent(in) :: omega(pcols,pver)          ! vertical velocity
   ! --- Yi-Chi
...
! +++ Yi-Chi : relax PBL constraint of ZM
!              by setting pblh as 600hPa
  do k = pver - 1,msg + 1,-1
      do i = 1,ncol
         if ((p(i,k).le.600) .and. (p(i,k+1).gt.600)) pblt(i) = k
      end do
   end do
!   do k = pver - 1,msg + 1,-1
!      do i = 1,ncol
...
!
   ...
   if( cam_physpkg_is('cam3')) then
      ...
      call buoyan_dilute(lchnk   ,ncol    , &
                  ...
                  pblt    ,lcl     ,lel     ,lon     ,maxi     , &
                  rgas    ,grav    ,cpres   ,msg     , &
! ++ Yi-Chi : add landfrac and omega for CIN constraint calculation
                  tpert   , landfrac, omega)
!                  tpert   )
! --- Yi-Chi
   end if
...
! +++ Yi-Chi : June 2016 +++ !
! As CAPE is calculated for once,
! no need to find the maximum value.
   do i = 1,ncol
   cape(i) = capeten(i,1)
   lel(i) = lelten(i,1)
   end do
...
!
   ...
   ! +++ Yi-Chi : add local variables for triggering
   logical plgcthk(pcols)
   integer lfc(pcols)
   real(r8) cthk, esat, qsat, hmxsat
   real(r8) heo(pcols,pver)
   ! --- Yi-Chi
...
!!!   RBN 9/9/04   !!!
   ...
   call parcel_dilute(lchnk, ncol, msg, mx, p, t, q, tpert, tp, tpv, qstp, pl, tl, lcl)
    ...
    ! +++ Yi-Chi adds CIN constraint as triggering function
    !     by constraining LFC should be close to original level (i.e. < cthk)
    do i = 1,ncol

      cthk = 0.  ! initialize
      call calcin(landfrac(i),omega(i,lcl(i))/100.,cthk)
      ! find LFC
      hmxsat = cp*t(i,mx(i)) + grav*z(i,mx(i)) + rl*q(i,mx(i))
      plgcthk(i) = .true.
      !heo(i,lcl(i)) = cp*t(i,lcl(i)) + grav*z(i,lcl(i)) + rl*q(i,lcl(i))
      lfc(i) = lcl(i)
      do k = lcl(i),msg,-1
         esat = c1*exp(c2*(t(i,k)-tfreez)/(c3+t(i,k)-tfreez)) ! esat(T) in mb
         qsat = eps1*esat/(p(i,k)-esat)                      ! Sat. mixing ratio (in kg/kg).
         heo(i,k) = cp*t(i,k) + grav*z(i,k) + rl*qsat
         if (plgcthk(i)) then
         if (k <= lcl(i) .and. hmxsat > heo(i,k)) then
            lfc(i) = k
            plgcthk(i) = .false.
         end if
         end if
      end do
      !  print *, '(Yi-Chi:zm_conv) i, lfc,lcl',i,lfc(i),lcl(i)
      if((p(i,mx(i))-p(i,lfc(i))) .gt. cthk) then
      plgcthk(i) = .false.
      else
      plgcthk(i) = .true.
      endif
      !print *, '(Yi-Chi:zm_conv) i, p-pl,lcl',i,p(i,mx(i))-pl(i),lcl(i)
      !print *, '(Yi-Chi:zm_conv) i, omega,cthk',i,omega(i,lcl(i)),cthk
      !print *, '(Yi-Chi:zm_conv) omega',omega
    !  print *, '(Yi-Chi:zm_conv) i, p-pl',i,p(i,mx(i))-pl(i)
    end do
...
!
   do k = pver,msg + 1,-1
      do i=1,ncol
      ! +++ Yi-Chi : add flg of CIN constraint(plgcthk) Sept 2013
     if (k <= mx(i) .and. plge600(i) .and. plgcthk(i)) then
...
!
   do k = msg + 2,pver
      do i = 1,ncol
        ! +++ Yi-Chi : add CIN constraint flag
        if (k < lcl(i) .and. plge600(i) .and. plgcthk(i)) then
        ! if (k < lcl(i) .and. plge600(i)) then
        ! --- Yi-Chi
            if (buoy(i,k+1) > 0._r8 .and. buoy(i,k) <= 0._r8) then
               knt(i) = min(5,knt(i) + 1)
...
!
   do n = 1,5
      do k = msg + 1,pver
         do i = 1,ncol
           ! +++ Yi-Chi : add CIN constraint flag
           if (plge600(i) .and. plgcthk(i) .and. k <= mx(i) .and. k > lelten(i,n)) then
           ! if (plge600(i) .and. k <= mx(i) .and. k > lelten(i,n)) then
           ! --- Yi-Chi
               capeten(i,n) = capeten(i,n) + rd*buoy(i,k)*log(pf(i,k+1)/pf(i,k))
            end if
...
! +++ Yi-Chi : Sept 2013
!     adds function for CIN calculation.
subroutine calcin (slimsk,pdot,cincr)
real(r8),  intent(in)  :: slimsk  ! land mask
real(r8), intent(in)  :: pdot    ! vertical velocity at cloud base
real(r8), intent(out) :: cincr   ! cin criteria

real(r8) w1l, w2l, w3l, w4l, w1s, w2s, w3s, w4s
real(r8) w1, w2, w3, w4
real(r8) val1, val2, tem, tem1
real(r8) cincrmax, cincrmin
     cincrmax=180.
     cincrmin=120.

    if(slimsk.gt.0.5) then
      tem = 1. ! for land
    else
      tem = 0. ! for water
    endif
