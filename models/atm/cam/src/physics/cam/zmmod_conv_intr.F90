...
! ++ Yi-Chi : April 2015
!       modify some part to add ZMMOD as an option in TaiESM.
! +++ Yi-Chi: Oct 2014
!       adds omega input for CIN constraint
! +++ Yi-Chi : April 2015
!       modify to ZMMOD
!---------------------------------------------------------------------------------
   ...
   use zm_conv,      only: zm_conv_evap, convtran, momtran
   use zmmod_conv,   only: zmmod_convr
   ! -------------- !
   ...
   ! Public methods
   public ::&
      zmmod_conv_init,               &! initialize donner_deep module
      zmmod_conv_tend,               &! return tendencies
      zmmod_conv_register,           &! register fields in physics buffer
      zmmod_conv_tend_2
   !public ::&
   !   zmmod_conv_register,           &! register fields in physics buffer
   !   zmmod_conv_init,               &! initialize donner_deep module
   !   zmmod_conv_tend!,               &! return tendencies
   !   !zmmod_conv_tend_2               ! return tendencies
...
subroutine zmmod_conv_register
...
end subroutine zmmod_conv_register

!=========================================================================================

subroutine zmmod_conv_init(pref_edge)
...
!----------------------------------------
  ...
  use zmmod_conv,        only: zmmod_convi
...
!
    ...
    if (masterproc) then
       write(iulog,*)'ZMMOD_CONV_INIT: Deep convection will be capped at intfc ',limcnv, &
            ' which is ',pref_edge(limcnv),' pascals'
    end if
        
    no_deep_pbl = phys_deepconv_pbl()
    !call zm_convi(limcnv,no_deep_pbl_in = no_deep_pbl)
    call zmmod_convi(limcnv,no_deep_pbl_in = no_deep_pbl)
...
end subroutine zmmod_conv_init
!=========================================================================================
!subroutine zm_conv_tend(state, ptend, tdt)

subroutine zmmod_conv_tend(pblh    ,mcon    ,cme     , &
     tpert   ,dlf     ,pflx    ,zdu      , &
     rliq    , &
     ztodt   , &
     jctop   ,jcbot , &
   ...
   lq(:) = .FALSE.
   lq(1) = .TRUE.
   call physics_ptend_init(ptend_loc, state%psetcols, 'zmmod_convr', ls=.true., lq=lq)! initialize local ptend type
...
!
   call t_startf ('zmmod_convr')

   call zmmod_convr(   lchnk   ,ncol    , &
...
! +++ Yi-Chi : Sept 2013 : including vertical velocity
                   lengath(lchnk) ,ql      ,rliq  ,landfrac   , state%omega)
...
!    do i = 1,nco
  ...
  lq(:) = .FALSE.
  lq(1) = .TRUE.
  call physics_ptend_init(ptend_loc, state1%psetcols, 'zmmod_conv_evap', ls=.true., lq=lq)

   call t_startf ('zmmod_conv_evap')
!
...
end subroutine zmmod_conv_tend
!=========================================================================================


subroutine zmmod_conv_tend_2( state,  ptend,  ztodt, pbuf)
...
end subroutine zmmod_conv_tend_2

!=========================================================================================



end module zmmod_conv_intr
