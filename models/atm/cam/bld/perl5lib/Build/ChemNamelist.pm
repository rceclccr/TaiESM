...
sub set_dep_lists
{
    ...
    my ( $gas_wetdep_list, $aer_wetdep_list, $drydep_list, $mam_drydep_list, $snap_drydep_list ) ;
...
#add SNAP by ICTsai
    if ( $chem_pkg =~ '_snap' or $chem_pkg =~ '_snap_mix') {

        $snap_drydep_list = get_snapdep_list( $cfgdir, $print_lvl, @species_list );

        if (length($aer_wetdep_list)>2) {
            $aer_wetdep_list = $aer_wetdep_list . "," . $snap_drydep_list;
        } else {
            $aer_wetdep_list = $snap_drydep_list;
        }

    }

    return (  $gas_wetdep_list, $aer_wetdep_list, $drydep_list, $mam_drydep_list, $snap_drydep_list );
}
...
# add SNAP by ICTsai
sub get_snapdep_list
{
    my ($cfg_dir,$print_lvl,@species_list) = @_;

    my $master_file = "$cfg_dir/namelist_files/master_snap_dep_list.xml";

    my $list = get_dep_list($master_file,$print_lvl,@species_list);

    if ($print_lvl>=2) {print " gas SNAP dep list : $list  \n" ;}
